#include "../../../include/planner/load_area_planning/load_area_planning.h"
...
std::tuple<bool, GlobalPlanning::Point, GlobalPlanning::Path, GlobalPlanning::Path, GlobalPlanning::Path> LoadAreaPlanning::LoadAreaPlanningInterface(int planning_mode, const GlobalPlanning::Point& wait_point, const GlobalPlanning::Point& load_point, const GlobalPlanning::Path& in_path, GlobalPlanning::Path out_path, GlobalPlanning::CollisonCheck& collision_checker) {
    FittingPathGenerate::FittingPathGenerator fit_path_planner(load_path_straight_length_weight_, load_path_curvature_weight_, out_put_path_dense_, search_range_, jump_dense_, length_weight_, critical_length_, curvature_weight_, min_straight_length_depart_, max_straight_length_depart_, delta_straight_length_depart_, min_straight_length_wait_, max_straight_length_wait_, delta_straight_length_wait_, min_straight_length_load_, max_straight_length_load_, delta_straight_length_load_);
    ...
    if (depart_path.empty()) {
        return std::make_tuple(false, queue_point, wait_path, load_path, depart_path);
    }
    if (planning_mode == 1) { // 尝试人工指定排队点
        ...
        return std::make_tuple(true, wait_point, wait_path, load_path, depart_path);
    }
    if (planning_mode == 2) {
        load_path = fit_path_planner.LoadPathGenerateInterface(load_point, wait_point, collision_checker).first;
        if (load_path.empty()) {
    ...
    if (planning_mode == 0) {
        WaitPointGenerate::WaitPointGenerator wait_point_planner(max_curve_length_, min_curve_length_, delta_curve_length_, wheel_base_length_, center2side_, max_straight_length_, min_straight_length_, delta_straight_length_, max_steering_angle_, min_steering_angle_, delta_steering_angle_, standard_steering_angle_, weight_length_, weight_curve_);
        load_path = wait_point_planner.GenerateWaitPointInterface(load_point, depart_path, collision_checker);
        if (load_path.empty()) {
            return std::make_tuple(false, queue_point, wait_path, load_path, depart_path);
        }
        ...
        return std::make_tuple(true, queue_point, wait_path, load_path, depart_path);
    }
}
