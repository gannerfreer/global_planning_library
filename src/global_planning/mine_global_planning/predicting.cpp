...
bool Predicting::InitialFunction() {
    ...
    all_referencelines_for_self_driving_.clear();
    global_path_.clear();
...
bool Predicting::ReadAllMapFile() {
    ...
    // 读取地图信息
    if (!configio_.GetMap(road_directed_graph_, map_border_, all_referencelines_for_self_driving_, sequence_mapping_, m_tar_rviz_data_) || !configio_.GetVehicleParam(vehicle_param_)) {
        return false;
    }
    else {
...
void Predicting::GlobalPathPlanningIntface(vector<_TrajectoryPoint>& path) {
...
PlanResult Predicting::ProgressiveHybirdAStar(_SinglePoint& input_point, int& search_index, vector<_TrajectoryPoint>& result_trajectory, const PlanRule& rule_id, int max_search_index) {
...
PlanResult Predicting::ApplyHibridAStarWithTime(_SinglePoint s_point, _SinglePoint e_point, vector<_TrajectoryPoint>& traj, const PlanRule& plan_rule_id, long long time_threshold) {
    // 起始点、目标点结构转换
...
// 成功：true，失败：false
bool Predicting::RandomOffsetWithoutCuravture() {
...
float Predicting::CalculateOffSetWithoutCuravture(int index, int sum, float weight) {
...
float Predicting::WeightFunction(int k, int sum) {
    int sita = 5;
    int x;
    if (k < sum / 2) {
        x = k;
...
// 全局轨迹规划--路径规划
PlanResult Predicting::PathPlanning() {
    ...
    if (task_type_ == TaskType::TEMP_MOVE_CAR) {
        ...
        int    end_path_id   = Helper::GetNearestReferencelines(end_point_, all_referencelines_for_self_driving_, temp_end_lat_dis, temp_end_lon_dis);
        int    start_path_id = Helper::GetNearestReferencelines(start_point_, all_referencelines_for_self_driving_, temp_start_lat_dis, temp_start_lon_dis);
...
// 非调度规划任务
PlanResult Predicting::NotFollowReferencelinePlanning() {
...
// 沿路网路径规划
PlanResult Predicting::FollowReferencelinePlanning() {
    ...
    Helper::GetReferencelinesWithRadiusAndAngle(end_point_, all_referencelines_for_self_driving_, end_search_radius, end_path_vec);
    ...
    while (start_search_radius <= 100) {
        if (Helper::GetReferencelinesWithRadius(start_point_, all_referencelines_for_self_driving_, start_search_radius, start_path_vec)) {
    ...
    for (auto& i : record) {
        ...
        temp_start_traj = all_referencelines_for_self_driving_.at(temp_start_key);
        temp_end_traj   = all_referencelines_for_self_driving_.at(temp_end_key);
    ...
    temp_start_traj = all_referencelines_for_self_driving_.at(start_key_);
    temp_end_traj   = all_referencelines_for_self_driving_.at(end_key_);
...
bool Predicting::HasSearched(int start, int end) {
...
bool Predicting::IsConnect(int start, int end) {
...
void Predicting::StartEndPointProcess() {
    // 计算全局路径第一个点与起点的角度偏差
...
void Predicting::PathClipAndSplice() {
    if (road_sequence_.size() == 1) {
        ...
        if (start_index_ <= end_index_) {
            global_path_.insert(global_path_.end(), all_referencelines_for_self_driving_.at(temp_key).trajectory.begin() + start_index_, all_referencelines_for_self_driving_.at(temp_key).trajectory.begin() + end_index_ + 1);
        }
        else // 同一路段，支持短距离倒车
        {
            threadLogger_->info("短距离倒车");
            global_path_.insert(global_path_.end(), all_referencelines_for_self_driving_.at(temp_key).trajectory.begin() + end_index_, all_referencelines_for_self_driving_.at(temp_key).trajectory.begin() + start_index_ + 1);
        ...
        for (int i = 0; i < road_sequence_.size(); i++) {
            ...
            if (i == 0) {
                global_path_.insert(global_path_.end(), all_referencelines_for_self_driving_.at(temp_key).trajectory.begin() + start_index_, all_referencelines_for_self_driving_.at(temp_key).trajectory.end());
            }
            else if (i == road_sequence_.size() - 1) {
                global_path_.insert(global_path_.end(), all_referencelines_for_self_driving_.at(temp_key).trajectory.begin(), all_referencelines_for_self_driving_.at(temp_key).trajectory.begin() + end_index_);
            }
            else {
                global_path_.insert(global_path_.end(), all_referencelines_for_self_driving_.at(temp_key).trajectory.begin(), all_referencelines_for_self_driving_.at(temp_key).trajectory.end());
            }
        }
    }
...
PlanResult Predicting::HybirdAStarFitting() {
...
bool Predicting::JudgeFittingDirection() {
...
bool Predicting::IsShortDistance() {
...
bool Predicting::PoseVerificationInterface(const _SinglePoint& start_pose, const _SinglePoint& end_pose, const bool flag) {
...
float Predicting::ReferencelineTotalDis(pair<int, int>& input_pair, int start_index, int end_index) {
    ...
    if (road_sequence_.size() > 1) {
        for (int i = 0; i < road_sequence_.size(); i++) {
            int temp_key = sequence_mapping_.at(road_sequence_.at(i));
            if (i == 0) {
                total_length += all_referencelines_for_self_driving_.at(temp_key).trajectory.size() - start_index;
                // threadLogger_->info("i:{}  total_length:{} all_referencelines_for_self_driving_.at(temp_key).trajectory.size():{} start_index:{}", i, total_length, all_referencelines_for_self_driving_.at(temp_key).trajectory.size(), start_index);
            }
            else if (i == road_sequence_.size() - 1) {
                ...
                total_length += all_referencelines_for_self_driving_.at(temp_key).trajectory.size();
                // threadLogger_->info("i:{}  total_length:{} all_referencelines_for_self_driving_.at(temp_key).trajectory.size():{}", i, total_length, all_referencelines_for_self_driving_.at(temp_key).trajectory.size());
            }
        }
        return total_length;
    }
...
void Predicting::CalculateCubicSplineCurve(bool flag, const Path& points, Path& cubicspline_path) {
    vector<double> x_set;
    vector<double> y_set;
...
void Predicting::CalculateStation(const vector<double>& xs, const vector<double>& ys) {
    double cum = 0.0;
    s_.clear();
    s_.push_back(cum);
...
void Predicting::CurvatureCal(vector<_TrajectoryPoint>& input_path) {
...
void Predicting::SmoothPath(vector<_TrajectoryPoint>& input_path) {
    // 得到节点和固定点索引
...
void Predicting::FillErrorCode(PlanResult result) {
...
vector<unsigned int> Predicting::CurvatureCheck(vector<_TrajectoryPoint>& input_path) {
