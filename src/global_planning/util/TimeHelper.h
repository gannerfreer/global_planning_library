...
namespace utility
{
	...
	public:
		...
		string ToString()
		{
			...
			sprintf(tmp, "%04d-%02d-%02d %02d:%02d:%02d", area->tm_year + 1900, area->tm_mon + 1,
	...
	public:
		...
		static string GetTimeString(time_t ts = 0)
		{
			...
			struct tm* area = localtime(&ts);
			sprintf(tmp, "%d-%d-%d %d:%d:%d", area->tm_year + 1900, area->tm_mon + 1,
				area->tm_mday, area->tm_hour, area->tm_min, area->tm_sec);
			return tmp;
		}
	};
	...
	public:
		...
		std::string GetLog() { flag_ = true; return str_log_; }
	...
	public:
		...
		~CTimeCounter()
		{
			cout << str_header_ << "\tcounter:" << to_string(counter_) << "\tusing_time:"
				<< to_string(using_time_ / 1000.0) << "ms\tarvage_time:"
				<< to_string(using_time_ / 1000.0 / counter_) <<"ms"<< endl;
		}
		void SetHeader(string str_header) { str_header_ = str_header; }
		void AddCounter() { ++counter_; }
		void AddUsingTime(long long using_time) { using_time_ += using_time; }
		uint32 GetCounter() { return counter_; }
		long long GetUsingTime() { return using_time_; }
	protected:
