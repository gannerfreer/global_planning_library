...
public:
     ...
     * false: 优化失败
     */
    void OptimizePath(Path &original_path,
                      Path &opti_path,
...
private:
     ...
     * @return 返回说明：
     */
    void UpdateFixPointSet(const vector<uint32> cllision_point);
    /**
     * @brief 路径优化核心函数
     * @return 返回说明：无
     */
     ...
     * @return 返回说明：无
     */
    void CubicInterpolate2Point(const Point start_point,
     ...
     */
    inline bool IsCusp(uint32 n);
    /**
     * @brief 判断当前点是否为固定点函数
     * @param[in]  m 当前点索引值
     * @return 返回说明：是固定点则返回true
     */
    inline bool IsFixPoint(uint32 m);

    vehicle_param m_vehicle_param_;

private:
    unordered_set<uint32> cusp_set_;     // 存放尖点索引
    unordered_set<uint32> fixpoint_set_; // 存放固定点索引
    Path path_;                          // 原路径
