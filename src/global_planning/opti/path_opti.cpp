 ...
 *return
 */
void Path_Opti::OptimizePath(Path &original_path,
                             Path &opti_path,
                             ...
                             vehicle_param m_vehicle_param)
{
    ...
    for (uint32 i = 0; i < original_path.size() - 1; ++i)
    {
        float64 dis_square = pow(original_path.at(i).x - original_path.at(i + 1).x, 2) +
...
void Path_Opti::GetFixPointIndex()
{
    ...
    for (auto it = cusp_set_.begin(); it != cusp_set_.end(); ++it)
    {
        ...
        // for(uint32 index = *it; index < path_.size()-1; index++)
        // {
 ...
 *return
 */
void Path_Opti::UpdateFixPointSet(const vector<uint32> cllision_point)
{
    // 将所有碰撞点变为固定点
    for (uint32 i = 0; i < cllision_point.size(); i++)
    {
        uint32 index = cllision_point.at(i);
...
void Path_Opti::SmoothPath()
{
    int32 L = path_.size();
    float64 x_sat = 6;
    vector<float64> coeff;
    for (int32 i = 0; i < L; i++)
    {
        float64 x = (i < L / 2) ? i : (L - 1.0 - i);
        float64 temp = 1 / (1 + exp(-x + x_sat));
    ...
    {
        for (uint32 i = 2; i < new_path_.size() - 2; i++)
        {
...
inline Vector2D Path_Opti::CurvatureTerm(Vector2D xim1, Vector2D xi, Vector2D xip1)
{
    ...
    float64 norm_delta_xi, norm_delta_xip1, d, dphi, kappa;
    ...
    {
        float64 pdphi_pcosdphi = -1 / sqrt(1 - pow(cos(dphi), 2));
        float64 u = pdphi_pcosdphi / norm_delta_xi;
        float64 s = dphi / pow(norm_delta_xi, 2);
...
{
    Vector2D oc;
    float64 c = (a.x * b.x + a.y * b.y) / (b.x * b.x + b.y * b.y);
    oc.x = a.x - c * b.x;
    oc.y = a.y - c * b.y;
    return oc;
}
...
void Path_Opti::CuspPointExtension(CollisonCheck &collison_check)
{
    ...
    for (uint32 i = 1; i < path_.size(); i++)
    {
        ...
        else
        {
            ...
            // 尖点处两点距离较近，则舍去一个点
            float64 dis_square = pow(path_.at(i).x - path_.at(i - 1).x, 2) +
                                 pow(path_.at(i).y - path_.at(i - 1).y, 2);
            if (dis_square < pow(0.5 * m_vehicle_param_.step_length, 2))
            {
                temp_path.pop_back();
            }
            float64 flag_pos_neg = (path_.at(i).direction == Forward) ? -1.0 : 1.0;
            uint32 num = 0;
            Point temp_point;

            // 尖点前直线延伸，最大延伸距离为cusp_extension_distance
            for (int32 j = 0; j <= m_vehicle_param_.cusp_extension_distance; j++)
            {
...
void Path_Opti::CalculatePathAngle()
{
    ...
    for (uint32 i = 1; i < new_path_.size() - 1; i++)
    {
        if (IsCusp(i))
        {
            continue;
        }
        float64 dx = (new_path_.at(i + 1).x - new_path_.at(i - 1).x);
        float64 dy = (new_path_.at(i + 1).y - new_path_.at(i - 1).y);
        float64 angle = atan(dy / dx);
        if (dx < 0)
            angle = angle + M_PI;
        else if (dx >= 0 && dy < 0)
            angle = angle + 2 * M_PI;
...
//     for (uint32 i = 0; i < new_path_.size() - 1; i++)
//     {
...
void Path_Opti::InterpolatePath(Path &opti_path)
{
    ...
    // Path copy_temp_path;
    // bool flag = true;
    // for (uint32 i = 0; i < vec_path.size(); i++)
    // {
    ...
    for (uint32 i = 0; i < vec_path.size(); i++)
    {
        for (uint32 j = 0; j < vec_path.at(i).size(); j++)
        {
...
void Path_Opti::CalculateCubicSplineCurve(bool flag, const Path &points, Path &cubicspline_path)
{
    ...
    for (float64 s = 0.0; s <= s_.back(); s += kDeltaS)
    {
...
void Path_Opti::CalculateStation(const std::vector<float64> &xs, const std::vector<float64> &ys)
{
    float64 cum = 0.0;
    s_.clear();
    s_.push_back(cum);
 ...
 *return
 */
void Path_Opti::CubicInterpolate2Point(const Point start_point,
                                       const Point end_point,
                                       const float64 delta_s,
                                       Path &interpolate_path)
{
    // cout << "start_point.x:" << start_point.x << " start_point.y:" << start_point.y << " end_point.x:" << end_point.x << " end_point.y:" << end_point.y << endl;
    float64 x1_t = end_point.x - start_point.x;
    float64 y1_t = end_point.y - start_point.y;
    float64 x1_t_r = x1_t * cos(start_point.angle) + y1_t * sin(start_point.angle);
    float64 y1_t_r = -x1_t * sin(start_point.angle) + y1_t * cos(start_point.angle);
    float64 a = tan(end_point.angle - start_point.angle) / x1_t_r / x1_t_r - 2 * y1_t_r / x1_t_r / x1_t_r / x1_t_r;
    float64 b = 3 * y1_t_r / x1_t_r / x1_t_r - tan(end_point.angle - start_point.angle) / x1_t_r;
    float64 x_i;
    float64 y_i;
    float64 theta_i;
    float64 x_i_r;
 ...
 * @brief 判断当前点是否为尖点函数
 */
inline bool Path_Opti::IsCusp(uint32 n)
{
 ...
 * @brief 判断当前点是否为固定点函数
 */
inline bool Path_Opti::IsFixPoint(uint32 m)
{
