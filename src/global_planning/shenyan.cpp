...
void GetMap(char *parea)
{
    ...
    if (doc.HasMember("all_zone_atr"))
    {
        ...
        }
        {
            ...
            m_map_Area.all_zone_atr.insert(m_map_Area.all_zone_atr.end(), retarea.all_zone_atr.begin(), retarea.all_zone_atr.end());
            cout << "GetMap函数内" << m_map_Area.all_zone_atr.size() << endl;
        }
    }
...
bool Shenyan::ReadAllMapFile()
{
    ...
    // 读取地图信息
    if (!configio_.GetMap(all_zones_, road_directed_graph_, m_tar_rviz_data_) || !configio_.GetVehicleParam(m_vehicle_param_))
    {
        return false;
    }
    else
    {
        task_type_ = m_vehicle_param_.task_type;
        return true;
    }
...
char *GlobalPathPlanning(char *point_veh_start_end)
{
    ...
    veh_start_end = dll.ParseInputInfo(point_veh_start_end);
    dll.key_ = veh_start_end.my_key;
    filemutex1.unlock();

    std::string vehicle_code = std::to_string(veh_start_end.m_Veh_Param.vehicle_code);

    dll.Vehicle_code_ = vehicle_code;
    ...
    try
    {
        dll.threadLogger_ = spdlog::rotating_logger_mt(vehicle_code, filePath, 10 * 2048 * 2048, 5, true);
        dll.threadLogger_->flush_on(spdlog::level::info);
        dll.my_optimal_path_.threadLogger_ = dll.threadLogger_;
        dll.threadLogger_->info(vehicle_code);
        dll.threadLogger_->info("point_veh_start_end.strlen().size:{}", strlen(point_veh_start_end));
        dll.threadLogger_->info("veh_start_end.my_key:{}", veh_start_end.my_key);
        dll.threadLogger_->info("GlobalPathPlanning-IDS_Global_Planning_version: G_V1.0.1.20240702_RC");
    }
    catch (const std::exception &e)
    {
        cout << "异常赋值" << endl;
        // dll.threadLogger_->close();
        // dll.threadLogger_->info("temp_zone_id:{}-------all_zones_.size():{}", temp_zone_id, dll.all_zones_.size());
        dll.error_type_ = static_cast<uint>(ErrorType::ALGORITHM_ERROR);
        {
            std::unique_lock<std::shared_mutex> lock(return_receive_ptr_mutex);

            string temp_string = dll.VecWaypoint2json(path);
...

    {
        ...
        dll.threadLogger_->info("m_map_Area.Road_Directed_Graph.graph.size():{}", m_map_Area.Road_Directed_Graph.graph.size());
        dll.threadLogger_->info("m_map_Area.all_zone_atr.size():{}", m_map_Area.all_zone_atr.size());
        if (m_map_Area.Road_Directed_Graph.graph.size() == 0)
        {
            dll.error_type_ = static_cast<uint>(ErrorType::NO_MAP);
            {
                std::unique_lock<std::shared_mutex> lock(return_receive_ptr_mutex);
                dll.threadLogger_->info("进锁成功");
                string temp_string = dll.VecWaypoint2json(path);
                int temp_string_size = temp_string.size();
                ...
                if (temp_string_size < 10)
                {
                    ...
                    dll.threadLogger_->info("出锁成功");
                }
                dll.threadLogger_->info("VecWaypoint2json successfully");
                cout << "receive_ptr.strlen().size():" << strlen(receive_ptr) << endl;
                dll.threadLogger_->info("receive_ptr.strlen().size()::{}", strlen(receive_ptr));
                dll.threadLogger_->info("出锁成功");
                return receive_ptr;
            }
        }
        ...
        for (int i = 0; i < m_map_Area.all_zone_atr.size(); i++)
        {
            ...
            dll.threadLogger_->info("区域{}有{}条参考路径", i + 1, size);
            cout << "区域" << i + 1 << "有" << size << "条参考路径" << endl;
        }
        cout << "m_map_Area.Road_Directed_Graph.graph.size():" << m_map_Area.Road_Directed_Graph.graph.size() << endl;
        cout << "m_map_Area.all_zone_atr.size():" << m_map_Area.all_zone_atr.size() << endl;
        dll.road_directed_graph_ = m_map_Area.Road_Directed_Graph;
        dll.all_zones_ = m_map_Area.all_zone_atr;
    } // 获取传入的内边界并将其存入对应的r区域内
    uint8 temp_zone_id;
    cout << "成功创建日志对象" << endl;
    cout << "地图有" << dll.all_zones_.size() << "个区域" << endl;
        ...
        try
        {
            for (size_t i = 0; i < veh_start_end.inner_borders.size(); i++)
            {
                if (!veh_start_end.inner_borders.at(i).inner_border.empty())
                {
                    ...
                    dll.all_zones_.at(temp_zone_id - 1).inner_borders = veh_start_end.inner_borders.at(i).inner_border;
                    // dll.my_inner_border = veh_start_end.inner_borders.at(i).inner_border;
                }
            }
        }
        catch (const std::exception &e)
    ...
    {
        dll.threadLogger_->error("此次任务无inner_borders信息");
    }
    dll.task_type_ = veh_start_end.m_task_type;
    dll.m_vehicle_param_ = veh_start_end.m_Veh_Param;
    dll.threadLogger_->info("m_start_point.x ={}", veh_start_end.m_Start_Point.x);
    dll.threadLogger_->info("m_start_point.y ={}", veh_start_end.m_Start_Point.y);
    dll.threadLogger_->info("m_start_point.z ={}", veh_start_end.m_Start_Point.z);
    dll.threadLogger_->info("m_start_point.yaw ={}", veh_start_end.m_Start_Point.yaw);
    dll.threadLogger_->info("m_End_Point.x ={}", veh_start_end.m_End_Point.x);
    dll.threadLogger_->info("m_End_Point.y ={}", veh_start_end.m_End_Point.y);
    dll.threadLogger_->info("m_End_Point.z ={}", veh_start_end.m_End_Point.z);
    dll.threadLogger_->info("m_End_Point.yaw ={}", veh_start_end.m_End_Point.yaw);
    // 起点
    dll.start_point_ = veh_start_end.m_Start_Point;
    // 终点
    dll.end_point_ = veh_start_end.m_End_Point;
    // spdlog::shutdown();
    try
    {
        dll.GlobalPathPlanningIntface(path);
    }
    catch (const std::exception &e)
    {
        cout << "调用全局规划库内部出现异常" << endl;
        dll.threadLogger_->info("调用全局规划库内部出现异常");
        dll.error_type_ = static_cast<uint>(ErrorType::ALGORITHM_ERROR);
        {
            std::unique_lock<std::shared_mutex> lock(return_receive_ptr_mutex);
            dll.threadLogger_->info("进锁成功");
    ...
    try
    {
        ...
        dll.threadLogger_->info("最后一步，返回字符指针");
    ...
    catch (const std::exception &e)
    {
        ...
        dll.threadLogger_->info("全局轨迹规划库内部，返参出现异常");
        dll.error_type_ = static_cast<uint>(ErrorType::ALGORITHM_ERROR);
        {
            std::unique_lock<std::shared_mutex> lock(return_receive_ptr_mutex);
            dll.threadLogger_->info("进锁成功");
...
void Shenyan::GlobalPathPlanningIntface(std::vector<Trajectory_Point> &path)
{
    ...
    vehicle_commander_ = m_vehicle_param_.veh_center_2_front + m_vehicle_param_.veh_center_2_rear_bound; // 获取车长参数
    ...
    else
    {
        ...
        temp = global_path_;
        // 对参考线进行随机偏移
        if (m_vehicle_param_.offset_enable == true)
        {
        ...
        CalCurv(global_path_); // 计算路径的曲率信息,建议打开，基于map.json和三次样条插值算出的曲率来计算，而不是直接用他们原始的曲率，因为三次样条插值算出的曲率变化不连续
        ...
        if (!my_speed_planning_.SpeedPlanning(global_path_, m_vehicle_param_))
        {
        ...
        // file_out.open("speed.txt");
        // for (size_t index = 0; index < global_path_.size(); index++)
        // {
        //     file_out << 0.0 << " " << global_path_.at(index).speed << endl;
        // }
        // file_out.close();
        // file_out.open("speed_limit.txt");
        // for (size_t index = 0; index < global_path_.size(); index++)
        // {
        //     file_out << 0.0 << " " << global_path_.at(index).speed_limit << endl;
        // }
        // file_out.close();
        // file_out.open("curvature.txt");
        // for (size_t index = 0; index < global_path_.size(); index++)
        // {
        //     file_out << 0.0 << " " << global_path_.at(index).curvature << endl;
        // }
        // file_out.close();
        // // 将加速度保存下来
        // CalAcc(global_path_);
        // file_out.open("acc_information.txt");
        // for (size_t index = 0; index < global_path_.size(); index++)
        // {
        //     file_out << 0.0 << " " << global_path_.at(index).acc << endl;
        // }
        // file_out.close();
        last_task_type_ = task_type_;
        return;
...
void Shenyan::Calrad2deg(std::vector<Trajectory_Point> &temp_path)
{
 ...
 * @return 实现结果
 */
void Shenyan::ReplanPointMaxSpeed(std::vector<Trajectory_Point> &temp_path)
{
    ...
    for (; iter != temp_path.end(); iter++)
    {
        ...
        {
            // 此时，该点为前进路径上的点
            if (fabs(iter->curvature) < m_vehicle_param_.min_curvature)
            {
                // 其绝对值<0.001(近似直线段) 限速5m/s-8.5m/s
                iter->speed_limit = m_vehicle_param_.max_speed_limit;
            }
            else if (m_vehicle_param_.min_curvature <= fabs(iter->curvature) && fabs(iter->curvature) < m_vehicle_param_.max_curvature)
            {
                // 其绝对值>0.01 <0.1 限速4m/s
                iter->speed_limit = m_vehicle_param_.mid_speed_limit;
            }
            else if (fabs(iter->curvature) >= m_vehicle_param_.max_curvature)
            {
                // 其绝对值>0.01 限速3m/s
                iter->speed_limit = m_vehicle_param_.min_speed_limit;
            }
        }
        else
        {
            // 此时，该点为后退路径上的点,后退轨迹限速均为2m/s
            iter->speed_limit = m_vehicle_param_.reverse_speed;
        }
    }
    // 基于道路属性的速度约束
    if (m_vehicle_param_.multi_section_speed_limit_enable)
    {
        threadLogger_->info("基于路况的分段速度规划开启");

        iter = temp_path.begin();
        int tasktype = 1;
        if (m_vehicle_param_.weather == 1) // 晴天
        {
            // 添加任务类型和路况决定的限速，与上面曲率计算出来的限速相结合，取小原则
            if (task_type_ == static_cast<uint>(TaskType::GO_LOAD_POINT) || task_type_ == static_cast<uint>(TaskType::GO_LOAD_TRANSITIONS_POINT) || task_type_ == static_cast<uint>(TaskType::GO_LOADAREA_WAITAREA) || task_type_ == static_cast<uint>(TaskType::GO_PARKLOT_POINT) || task_type_ == static_cast<uint>(TaskType::GO_PARK_AREA)) // 空载情况
            {
                for (; iter != temp_path.end(); iter++)
                {
                    if (iter->direction == 0)
                    {
                        ...
                        {
                            if (iter->speed_limit > m_vehicle_param_.light_regular_road_speed_limit)
                                iter->speed_limit = m_vehicle_param_.light_regular_road_speed_limit;
                        }
                        else if (iter->attribute == 1) // 窄路
                        {
                            if (iter->speed_limit > m_vehicle_param_.light_narrow_road_speed_limit)
                                iter->speed_limit = m_vehicle_param_.light_narrow_road_speed_limit;
                        }
                        else if (iter->attribute == 2) // 路口
                        {
                            if (iter->speed_limit > m_vehicle_param_.light_intersection_road_speed_limit)
                                iter->speed_limit = m_vehicle_param_.light_intersection_road_speed_limit;
                        }
                        else if (iter->attribute == 3) // 坡路
                        {
                            if (iter->speed_limit > m_vehicle_param_.light_slope_road_speed_limit)
                                iter->speed_limit = m_vehicle_param_.light_slope_road_speed_limit;
                        }
                        else if (iter->attribute == 4) // 颠簸路
                        {
                            if (iter->speed_limit > m_vehicle_param_.light_bumpy_road_speed_limit)
                                iter->speed_limit = m_vehicle_param_.light_bumpy_road_speed_limit;
                        }
            ...
            else // 满载情况
            {
                for (; iter != temp_path.end(); iter++)
                {
                    if (iter->direction == 0)
                    {
                        if (iter->attribute == 0) // 正常路
                        {
                            if (iter->speed_limit > m_vehicle_param_.heavy_regular_road_speed_limit)
                                iter->speed_limit = m_vehicle_param_.heavy_regular_road_speed_limit;
                        }
                        else if (iter->attribute == 1) // 窄路
                        {
                            if (iter->speed_limit > m_vehicle_param_.heavy_narrow_road_speed_limit)
                                iter->speed_limit = m_vehicle_param_.heavy_narrow_road_speed_limit;
                        }
                        else if (iter->attribute == 2) // 路口
                        {
                            if (iter->speed_limit > m_vehicle_param_.heavy_intersection_road_speed_limit)
                                iter->speed_limit = m_vehicle_param_.heavy_intersection_road_speed_limit;
                        }
                        else if (iter->attribute == 3) // 坡路
                        {
                            if (iter->speed_limit > m_vehicle_param_.heavy_slope_road_speed_limit)
                                iter->speed_limit = m_vehicle_param_.heavy_slope_road_speed_limit;
                        }
                        else if (iter->attribute == 4) // 颠簸路
                        {
                            if (iter->speed_limit > m_vehicle_param_.heavy_bumpy_road_speed_limit)
                                iter->speed_limit = m_vehicle_param_.heavy_bumpy_road_speed_limit;
                        }
                        else if (iter->attribute == 5) // 水泥路
                        {
                            if (iter->speed_limit > m_vehicle_param_.heavy_regular_road_speed_limit)
        ...
        {
            // 添加任务类型和路况决定的限速，与上面曲率计算出来的限速相结合，取小原则
            if (task_type_ == static_cast<uint>(TaskType::GO_LOAD_POINT) || task_type_ == static_cast<uint>(TaskType::GO_LOADAREA_WAITAREA) || task_type_ == static_cast<uint>(TaskType::GO_PARKLOT_POINT) || task_type_ == static_cast<uint>(TaskType::GO_PARK_AREA)) // 空载情况
            {
                for (; iter != temp_path.end(); iter++)
                {
                    if (iter->direction == 0)
    ...
    threadLogger_->info("s_curve_speed_limit:{}", m_vehicle_param_.s_curve_speed_limit);

    if (m_vehicle_param_.s_curve_speed_limit)
    {
        std::ofstream file_out;
...
string Shenyan::VecWaypoint2json(ReferenceTrajectory &vec_wp)
{
    ...
    for (size_t i = 0; i < vec_wp.size(); i++)
    {
        ...
        writer.Key("left");
        writer.Double(vec_wp.at(i).left);

        writer.Key("right");
        writer.Double(vec_wp.at(i).right);
        ...
        writer.Key("slope");
        writer.Double(vec_wp.at(i).slope);
    ...
    // 将JSON数据写入文件
    outputFile << g_StrData;
    // 关闭文件流
    outputFile.close();
    threadLogger_->info(" g_StrData = strbuf.GetString()");
    cout << "g_StrData = strbuf.GetString();" << endl;
...
// 区域内临时挪车
ErrorType Shenyan::PlanningCase7()
{
...
ErrorType Shenyan::PlanningCase8()
{
...
// 前往装载点
ErrorType Shenyan::PlanningCase9()
{

    threadLogger_->info("Enter PlanningCase9");

    global_path_.clear();
    // m_vehicle_param_.end_offset_distance = 4.0;
    // m_vehicle_param_.start_offset_distance = 4.0;
    std::vector<Trajectory_Point> temp_trajectory;
    // rule_id:5n前向规划失败，换rule_id:0的普通规划
...
ErrorType Shenyan::PlanningCase10() // 前往驶离装载点
{
 ...
 */
ErrorType Shenyan::PlanningCase1() // 暂时只考虑单车道的场景，后续扩展为多车道需修改函数PlanningCase1()到PlanningCase6()
{
    ...
    if (temp_all_trajectories.size()) // 区域内有参考路径才进行后续的规划
    {
        ...
        while (same_road_num > 1 && index_dis <= dis_v.size() - 1)
        {
            ...
            temp_error_type = FindReferencePath(start_point_, temp_all_trajectories, start_nearest_id, start_path_id, true, v_id_start, dis);
            if (temp_error_type != ErrorType::SUCCESS) // 找到区域内与起始点最近的参考路径
            {
            ...
            threadLogger_->info(" ");
            temp_error_type = FindReferencePath(end_point_, temp_all_trajectories, end_nearest_id, end_path_id, false, v_id_end, dis);
            if (temp_error_type != ErrorType::SUCCESS) // 找到区域内与目标点最近的参考路径
            {
        ...
        start_flag = DetermineIfOrNoHybirdAStar(start_point_, start_trajectory.trajectory, start_nearest_id, lat_distance);
        // 根据起点和终点
        ...
        if (temp_distance < 37)                                                      // 37m是矿卡能够正常规划的极限距离
        {
        ...
        // 起始点和目标点在同一路径上
        int32 Splice_Point_start_index = -1; // 起点横向偏离参考轨迹较大，拟合轨迹终点的索引，下一区域拼接轨迹时，轨迹拼接的起点
        int32 Splice_Point_end_index = -1;   // 终点横向偏离参考轨迹较大，拟合轨迹起点的索引，下一区域拼接轨迹时，轨迹拼接的终点
        ...
        if (end_nearest_id < start_nearest_id) // 需要倒车
        {
            ...
            else
            {
                ...
                reverse_tra.insert(reverse_tra.end(), temp_traj.begin() + end_nearest_id, temp_traj.begin() + start_nearest_id + 1);
                reverse(reverse_tra.begin(), reverse_tra.end());
                reverse_flag_ = true;
        ...
        else if (end_nearest_id > start_nearest_id) //  【起点、终点位于同一参考轨迹且起点在终点后】
        {
            ...
            if (start_flag)
            {
                ...
                // 参数顺序解释：输入点、hybird A*规划区域、搜索方向、在哪条轨迹上搜索、从当前轨迹上哪个索引处开始搜索、搜索到的轨迹点索引、hybirdA*最终拟合出的轨迹
                uint8 hybird_rule_id = 5;

                if (!ProgressiveHybirdAStar(start_point_, start_node_, true, start_trajectory.trajectory, start_nearest_id, Splice_Point_start_index, temp_tra_start, hybird_rule_id))
                {
            ...
            if (end_flag)
            {
                ...
                if (!ProgressiveHybirdAStar(end_point_, end_node_, false, end_trajectory.trajectory, end_nearest_id, Splice_Point_end_index, temp_tra_end, hybird_rule_id))
                {
            ...
            if (start_flag)
            {
                if (end_flag)
                {
                    ...
                    global_path_.insert(global_path_.end(), temp_traj.begin() + Splice_Point_start_index + 1, temp_traj.begin() + Splice_Point_end_index);
                    global_path_.insert(global_path_.end(), temp_tra_end.begin(), temp_tra_end.end());
                }
                else
                {
                    global_path_.insert(global_path_.end(), temp_tra_start.begin(), temp_tra_start.end());
                    global_path_.insert(global_path_.end(), temp_traj.begin() + Splice_Point_start_index + 1, temp_traj.begin() + end_nearest_id + 1);
                }
            }
            else
            {
                if (end_flag)
                {
                    global_path_.insert(global_path_.end(), temp_traj.begin() + start_nearest_id, temp_traj.begin() + Splice_Point_end_index);
                    global_path_.insert(global_path_.end(), temp_tra_end.begin(), temp_tra_end.end());
                }
                else
                {
                    global_path_.insert(global_path_.end(), temp_traj.begin() + start_nearest_id, temp_traj.begin() + end_nearest_id + 1);
                }
            }
        }
        else // 起点终点相距很近
        {
            ...
            if (fabs(angle - start_point_.yaw) / M_PI * 180 < 90 || fabs(angle - start_point_.yaw) / M_PI * 180 > 270)
            {
                p1.direction = 0;
                p2.direction = 0;
            }
 ...
 * @param [return] true: 规划成功；false：规划失败
 */
ErrorType Shenyan::PlanningCase2()
{
    ...
    uint8 rule;
    global_path_.clear();
    if (task_type_ == static_cast<uint>(TaskType::GO_LOADAREA_WAITAREA) || task_type_ == static_cast<uint>(TaskType::GO_UNLOADAREA_WAITAREA))
        rule = 5;
    else
        rule = 2;
 ...
 * @param [return] true: 规划成功；false：规划失败
 */
ErrorType Shenyan::PlanningCase3()
{
    ...
    if (start_all_trajectories.size() && end_all_trajectories.size()) // 区域内均有参考路径才进行后续的规划
    {
        ...
        temp_error_type = FindReferencePath(start_point_, start_all_trajectories, start_nearest_id, start_path_id, true, v_id, 0);
        if (temp_error_type != ErrorType::SUCCESS) // 找到区域内与起始点最近的参考路径
        {
        ...
        temp_error_type = FindReferencePath(end_point_, end_all_trajectories, end_nearest_id, end_path_id, false, v_id, 0);
        if (temp_error_type != ErrorType::SUCCESS) // 找到区域内与目标点最近的参考路径
        {
        ...
        threadLogger_->info("判断终点是否需要进行hybirdA*规划");
        bool end_flag = DetermineIfOrNoHybirdAStar(end_point_, end_trajectory.trajectory, end_nearest_id, lat_distance);
        threadLogger_->info("判断起点是否需要进行hybirdA*规划");
        bool start_flag = DetermineIfOrNoHybirdAStar(start_point_, start_trajectory.trajectory, start_nearest_id, lat_distance);
        ...
        Single_Point input_point;                                         // ProgressiveHybirdAStar 渐进式hybird A*搜索函数输入形参-规划输入点
        uint8 search_zone;                                                // ProgressiveHybirdAStar 渐进式hybird A*搜索函数输入形参-当前规划区域
        vector<Trajectory_Point> Trajectory_be_searched;                  // ProgressiveHybirdAStar 渐进式hybird A*搜索函数输入形参-在哪条参考轨迹上搜索hybird A*拟合点
        uint32 serach_start_index = 0;                                    // ProgressiveHybirdAStar 渐进式hybird A*搜索函数输入形参-从哪个索引处开始搜索
        std::vector<Trajectory_Point> temp_tra_start, temp_tra_end;       // ProgressiveHybirdAStar 渐进式hybird A*搜索函数输入形参-搜索出的轨迹
        int32 Splice_Point_start_index = -1, Splice_Point_end_index = -1; // ProgressiveHybirdAStar 渐进式hybird A*搜索函数输入形参-记录规划起点、规划终点处的hybird A*拼接轨迹分别记录规划起点、规划终点的hybird A*拼接点的索引
        ...
        {
            if (i == 0) // planningcase3为结构化区域到结构化区域，所以zone_sequence里面存的第一个区域一定是结构化区域,同理， 最后一个区域也是结构化区域
            {
                ...
                if (zone_sequence_.size() > 2) // 【当前区域：起点、结构化——下一个区域：非终点】
                {
                    ...
                    if (start_point_to_next_area) //  【当前区域：起点、结构化、 规划起点靠近下一个区域——下一个区域：非终点、结构化】
                    {
                        threadLogger_->info("【当前区域：起点、结构化、 规划起点靠近下一个区域——下一个区域：非终点、结构化】");
                        ...
                        else
                        {
                            ...
                            global_path_.insert(global_path_.end(), temp_traj.begin() + start_nearest_id, temp_traj.end());
                        }
                    }
                    else //   【当前区域：起点、结构化、 规划起点距离下一区域较远——下一个区域：非终点、结构化】
                    {
                        threadLogger_->info("【当前区域：起点、结构化、 规划起点距离下一区域较远——下一个区域：非终点、结构化】");
                        ...
                        if (start_flag)
                        {
                            ...
                            if (!ProgressiveHybirdAStar(input_point, search_zone, true, Trajectory_be_searched, serach_start_index, Splice_Point_start_index, temp_tra_start, hybird_rule_id))
                            {
                                // 反馈规划起点附近hybird A*轨迹拼接失败
                        ...
                        {
                            global_path_.insert(global_path_.end(), temp_tra_start.begin(), temp_tra_start.end());
                            global_path_.insert(global_path_.end(), temp_traj.begin() + Splice_Point_start_index + 1, temp_traj.end());
                        }
                        else
                        {
                            global_path_.insert(global_path_.end(), temp_traj.begin() + start_nearest_id, temp_traj.end());
                ...
                else // 下一个区域是终点所在区域,在planningcase3中只可能是结构化区域 【当前区域：起点、结构化——————下一个区域：终点、结构化】
                {
                    threadLogger_->info("【当前区域：起点、结构化——————下一个区域：终点、结构化】");

                    if (start_point_to_next_area) // 【当前区域：起点、结构化、 起点靠近下一个区域——————下一个区域：终点、结构化】
                    {
                        threadLogger_->info("【当前区域：起点、结构化、 起点靠近下一个区域——————下一个区域：终点、结构化】");

                        if (end_point_to_pre_area) // 【当前区域：起点、结构化、 起点靠近下一个区域——————下一个区域：终点、结构化、 终点靠近上一个区域】
                        {
                            threadLogger_->info("【当前区域：起点、结构化、 起点靠近下一个区域——————下一个区域：终点、结构化、 终点靠近上一个区域】");

                            threadLogger_->info(" do nothing，此部分任务交于后续逻辑来 处理");
                        }
                        else // 【当前区域：起点、结构化、 起点靠近下一个区域——————下一个区域：终点、结构化、 终点远离前一个区域】
                        {
                            threadLogger_->info("【当前区域：起点、结构化、 起点靠近下一个区域——————下一个区域：终点、结构化、 终点远离前一个区域】");
                    ...
                    {
                        threadLogger_->info("【当前区域：起点、结构化、 起点远离下一个区域——————下一个区域：终点、结构化、直走】");

                        if (end_point_to_pre_area) // 【当前区域：起点、结构化、 起点远离下一个区域——————下一个区域：终点、结构化、 终点靠近上一个区域】
                        {
                            threadLogger_->info("【当前区域：起点、结构化、 起点远离下一个区域——————下一个区域：终点、结构化、 终点靠近上一个区域】");
                            ...
                            if (end_flag)
                            {
                                ...
                                if (!ProgressiveHybirdAStar(input_point, search_zone, false, Trajectory_be_searched, serach_start_index, Splice_Point_end_index, temp_tra_end, hybird_rule_id))
                                {
                                    // 反馈规划起点附近hybird A*轨迹拼接失败
                                    threadLogger_->error("规划起点附近hybird A*轨迹拼接失败");
                            ...
                            if (start_flag)
                            {
                                ...
                                {
                                    global_path_.insert(global_path_.end(), temp_tra_start.begin(), temp_tra_start.end());
                                    global_path_.insert(global_path_.end(), temp_traj.begin() + Splice_Point_start_index + 1, temp_traj.begin() + Splice_Point_end_index - 1);
                                    global_path_.insert(global_path_.end(), temp_tra_end.begin(), temp_tra_end.end());
                                }
                                else
                                {
                            ...
                            else
                            {
                                if (end_flag)
                                {
                                    global_path_.insert(global_path_.end(), temp_traj.begin() + start_nearest_id, temp_traj.begin() + Splice_Point_end_index - 1);
                                    global_path_.insert(global_path_.end(), temp_tra_end.begin(), temp_tra_end.end());
                                }
                                else
                                {
                        ...
                        else // 【当前区域：起点、结构化、 起点远离下一个区域——————下一个区域：终点、结构化、 终点远离上一个区域】
                        {
                            threadLogger_->info("【当前区域：起点、结构化、 起点远离下一个区域——————下一个区域：终点、结构化、 终点远离上一个区域】");
            ...
            else if ((zone_sequence_.size() - 1) == i) // 当前区域为最后一个结构化区域
            {
                ...
                if (zone_sequence_.size() == 2) // 【上一个区域：起点、结构化———当前区域：终点、结构化】
                {
                    ...
                    if (start_point_to_next_area) // 【上一个区域：起点、结构化、 规划起点靠近当前区域———当前区域：终点、结构化、直走】
                    {
                        threadLogger_->info("【上一个区域：起点、结构化、 规划起点靠近当前区域———当前区域：终点、结构化、直走】");

                        if (end_point_to_pre_area) // 【上一个区域：起点、结构化、 规划起点靠近当前区域———当前区域：终点、结构化、 规划终点靠近上一个区域】
                        {
                            threadLogger_->info("【上一个区域：起点、结构化、 规划起点靠近当前区域———当前区域：终点、结构化、 规划终点靠近上一个区域】");
                            if ((!start_flag) && (!end_flag))
                            {
                                global_path_.insert(global_path_.end(), start_trajectory.trajectory.begin() + start_nearest_id, start_trajectory.trajectory.end());
                                global_path_.insert(global_path_.end(), end_trajectory.trajectory.begin(), end_trajectory.trajectory.begin() + end_nearest_id);
                                return ErrorType::SUCCESS;
                            }
                        ...
                        else // 【上一个区域：起点、结构化、 规划起点靠近当前区域———当前区域：终点、结构化、 规划终点远离上一个区域】
                        {
                            threadLogger_->info("【上一个区域：起点、结构化、 规划起点靠近当前区域———当前区域：终点、结构化、 规划终点远离上一个区域】");
                            ...
                            else
                            {
                                if (end_flag)
                                {
                                    threadLogger_->info("Splice_Point_end_index:{}", static_cast<int>(Splice_Point_end_index));

                                    global_path_.insert(global_path_.end(), temp_traj.begin(), temp_traj.begin() + Splice_Point_end_index);
                                    global_path_.insert(global_path_.end(), temp_tra_end.begin(), temp_tra_end.end());
                                }
                                else
                                {
                                    global_path_.insert(global_path_.end(), temp_traj.begin(), temp_traj.begin() + end_nearest_id + 1);
                                }
                            }
                        }
                    }
                    ...
                    {
                        threadLogger_->info("【上一个区域：起点、结构化、 规划起点远离当前区域———当前区域：终点、结构化、直走】");

                        if (end_point_to_pre_area) // 【上一个区域：起点、结构化、 规划起点远离当前区域———当前区域：终点、结构化、 规划终点靠近上一个区域】
                        {
                            threadLogger_->info("【上一个区域：起点、结构化、 规划起点远离当前区域———当前区域：终点、结构化、 规划终点靠近上一个区域】");
                        ...
                        else // 【上一个区域：起点、结构化、 规划起点远离当前区域———当前区域：终点、结构化、 规划终点远离上一个区域】
                        {
                            threadLogger_->info("【上一个区域：起点、结构化、 规划起点远离当前区域———当前区域：终点、结构化、 规划终点远离上一个区域】");
                            ...
                            if (end_flag)
                            {
                                ...
                                global_path_.insert(global_path_.end(), temp_traj.begin(), temp_traj.begin() + Splice_Point_end_index - 1);
                                global_path_.insert(global_path_.end(), temp_tra_end.begin(), temp_tra_end.end());
                            }
                            else
                            {
                ...
                else // 【上一个区域：非起点———当前区域：终点、结构化】
                {
                    ...
                    if (end_point_to_pre_area) // 【上一个区域：非起点、结构化———当前区域：终点、结构化、终点靠近上一个区域】
                    {
            ...
            else // 当前区域为中间的结构化区域 【上一个区域：未知———当前区域：常规、结构化——下一个区域：未知】
            {
                ...
                if (!FindPathId(zone_sequence_.at(i - 1), zone_sequence_.at(i), zone_sequence_.at(i + 1), all_zones_.at(zone_sequence_.at(i) - 1).reference_trajs, temp_path_id))
                {
                    threadLogger_->error(" The map file for zone{} does not have path for turn over", static_cast<float32>(zone_sequence_.at(i)));
                ...
                if (i != 1) // 【上一个区域：常规———当前区域：常规、结构化】
                {
                    ...
                    if (all_zones_.at(zone_sequence_.at(i - 1) - 1).zone_type == 1) // 【上一个区域：常规、结构化———当前区域：常规、结构化】
                    {
                ...
                else // 【上一个区域：起点、结构化———当前区域：常规、结构化】
                {
                    ...
                    if (start_point_to_next_area) // 【上一个区域：起点、结构化、 规划起点靠近当前区域———当前区域：常规、结构化】
                    {
                        threadLogger_->info("【上一个区域：起点、结构化、 规划起点靠近当前区域———当前区域：常规、结构化】");
                    ...
                    {
                        threadLogger_->info("【上一个区域：起点、结构化、 规划起点远离当前区域———当前区域：常规、结构化】");
                ...
                else // 【当前区域：常规、结构化——下一个区域：终点、结构化】
                {
                    ...
                    if (end_point_to_pre_area) // 【当前区域：常规、结构化——下一个区域：终点、结构化、规划终点靠近当前区域】
                    {
                ...
                if (has_start_hy)
                {
                    ...
                    {
                        global_path_.insert(global_path_.end(), temp_tra_start.begin(), temp_tra_start.end());
                        global_path_.insert(global_path_.end(), temp_traj.begin() + Crop_start_Point + 1, temp_traj.begin() + Crop_end_Point - 1);
                        global_path_.insert(global_path_.end(), temp_tra_end.begin(), temp_tra_end.end());
                    }
                    else
                    {
                        global_path_.insert(global_path_.end(), temp_tra_start.begin(), temp_tra_start.end());
                        global_path_.insert(global_path_.end(), temp_traj.begin() + Crop_start_Point + 1, temp_traj.end());
                    }
                }
                else
                {
                    if (has_end_hy)
                    {
                        global_path_.insert(global_path_.end(), temp_traj.begin(), temp_traj.begin() + Crop_end_Point - 1);
                        global_path_.insert(global_path_.end(), temp_tra_end.begin(), temp_tra_end.end());
                    }
                    else
                    {
 ...
 * @param [return] true: 规划成功；false：规划失败
 */
ErrorType Shenyan::PlanningCase4()
{
    ...
    if (start_all_trajectories.size()) // 区域内均有参考路径才进行后续的规划
    {
        ...
        for (size_t i = 0; i < zone_sequence_.size(); i++)
        {
            ...
            if (temp_area_type == 1)
            {
                if (i == 0) // planningcase4为结构化区域到非结构化区域，所以zone_sequence里面存的第一个区域一定是结构化区域
                {
                    threadLogger_->info("遍历到第{}个结构化区域*", i + 1);
                    ...
                    else // 下一个区域是终点所在区域,在planningcase4中只可能是非结构化区域 【当前区域：起点、结构化——————下一个区域：终点、非结构化】
                    {
                        threadLogger_->info("【当前区域：起点、结构化——————下一个区域：终点、非结构化】");

                        threadLogger_->info("【当前区域：起点、结构化、直走——————下一个区域：终点、非结构化】");

                        if (start_point_to_next_area) // 【当前区域：起点、结构化、 起点靠近下一个区域——————下一个区域：终点、非结构化】
                        {
                            threadLogger_->info("【当前区域：起点、结构化、 起点靠近下一个区域——————下一个区域：终点、非结构化】");

                            threadLogger_->info("do nothing");
                        }
                        else // 【当前区域：起点、结构化、 起点远离下一个区域——————下一个区域：终点、非结构化】
                        {
                            threadLogger_->info("【当前区域：起点、结构化、 起点远离下一个区域——————下一个区域：终点、非结构化】");
                            ...
                            float dis = sqrt(pow(temp_traj.at(nearest_id).x - end_point_.x, 2) + pow(temp_traj.at(nearest_id).y - end_point_.y, 2));
                ...
                else // 当前区域为中间的结构化区域 【上一个区域：未知———当前区域：常规、结构化——下一个区域：未知】
                {
                    ...
                    if (has_start_hy)
                    {
                        ...
                        {
                            global_path_.insert(global_path_.end(), temp_tra_start.begin(), temp_tra_start.end());
                            global_path_.insert(global_path_.end(), temp_traj.begin() + Crop_start_Point + 1, temp_traj.begin() + Crop_end_Point);
            ...
            else // 当前遍历到的区域为非结构化区域，在planningcase4中，非结构化区域只能是中间区域、或最后一个区域
            {
                if (i != zone_sequence_.size() - 1)
                {
                    ...
                    else // 【上一个区域：起点 、结构化——当前区域：常规、非结构化】
                    {
                        ...
                        if (start_point_to_next_area) // 【上一个区域：起点 、结构化、 规划起点靠近当前区域——当前区域：常规、非结构化】
                        {
                            threadLogger_->info("【上一个区域：起点 、结构化、 规划起点靠近当前区域——当前区域：常规、非结构化】");

                            temp_start_point = start_point_;
                        }
                        else // 【上一个区域：起点 、结构化、 规划起点远离当前区域——当前区域：常规、非结构化】
                        {
                            threadLogger_->info("【上一个区域：起点 、结构化、 规划起点远离当前区域——当前区域：常规、非结构化】");
                    ...
                    uint8 temp_path_id = 0;
                    if (!FindPathId(zone_sequence_.at(i), zone_sequence_.at(i + 1), zone_sequence_.at(i + 2), all_zones_.at(zone_sequence_.at(i + 1) - 1).reference_trajs, temp_path_id))
                    {
                        threadLogger_->error(" The map file for zone {} does not have path for turn over", static_cast<float32>(zone_sequence_.at(i + 1)));
                    ...
                    // std::vector<Trajectory_Point> temp_trajectory;
                    uint8 hybird_rule_id = 5;
                    if (!ProgressiveHybirdAStar(input_point, search_zone, true, Trajectory_be_searched, serach_start_index, Splice_Point_start_index, temp_trajectory, hybird_rule_id))
                    {
                        // 反馈规划起点附近hybird A*轨迹拼接失败
                        threadLogger_->error("规划起点附近hybird A*轨迹拼接失败");
                ...
                else // 最后一个非结构化区域
                {
                    ...
                    if (zone_sequence_.size() == 2) // 【上一个区域：起点、结构化———当前区域：终点、非结构化】
                    {
                        ...
                        if (start_point_to_next_area) // 【上一个区域：起点、结构化、 规划起点靠近当前区域———当前区域：终点、非结构化】
                        {
                            threadLogger_->info("【上一个区域：起点、结构化、 规划起点靠近当前区域———当前区域：终点、非结构化】");
                        ...
                        {
                            threadLogger_->info("【上一个区域：起点、结构化、 规划起点远离当前区域———当前区域：终点、非结构化】");
 ...
 * @param [return] true: 规划成功；false：规划失败
 */
ErrorType Shenyan::PlanningCase5()
{
    ...
    if (end_all_trajectories.size()) // 区域内均有参考路径才进行后续的规划
    {
        ...
        for (size_t i = 0; i < zone_sequence_.size(); i++)
        {
            ...
            if (temp_area_type == 0)
            {
                ...
                else // 下一个区域是终点所在区域,在planningcase5中只可能是结构化区域 【当前区域：起点、非结构化——————下一个区域：终点、结构化】
                {
                    threadLogger_->info("【当前区域：起点、非结构化——————下一个区域：终点、结构化】");

                    threadLogger_->info("【当前区域：起点、非结构化——————下一个区域：终点、结构化、直走】");

                    if (end_point_to_pre_area) // 【当前区域：起点、非结构化——————下一个区域：终点、结构化、 终点靠近前一个区域】
                    {
                        // std::vector<Trajectory_Point> temp_trajectory;
                    ...
                    {
                        threadLogger_->info("【当前区域：起点、非结构化——————下一个区域：终点、结构化、 终点远离前一个区域】");
            ...
            {
                if (i != zone_sequence_.size() - 1) // 当前区域为中间的结构化区域 【上一个区域：未知———当前区域：常规、结构化——下一个区域：未知】
                {
                    threadLogger_->info("遍历到第{} 个结构化区域", i + 1);
                    ...
                    else // 【当前区域：常规、结构化——下一个区域：终点、结构化】
                    {
                        ...
                        if (end_point_to_pre_area) // 【当前区域：常规、结构化——————下一个区域：终点、结构化、 终点靠近当前区域】
                        {
                            threadLogger_->info("【当前区域：常规、结构化——————下一个区域：终点、结构化、 终点靠近当前区域】");
                        ...
                        {
                            threadLogger_->info("【当前区域：常规、结构化——————下一个区域：终点、结构化、 终点远离前一个区域】");
                ...
                else // 最后一个结构化区域
                {
                    ...
                    if (zone_sequence_.size() == 2) // 【上一个区域：起点、非结构化———当前区域：终点、结构化】
                    {
                        ...
                        if (end_point_to_pre_area) // 【上一个区域：起点、非结构化———当前区域：终点、结构化、 规划终点靠近上一个区域】
                        {
                            threadLogger_->info("【上一个区域：起点、非结构化———当前区域：终点、结构化、 规划终点靠近上一个区域】");
                        ...
                        {
                            threadLogger_->info("【上一个区域：起点、非结构化———当前区域：终点、结构化、 规划终点远离上一个区域】");
                    ...
                    else // 【上一个区域：非起点———当前区域：终点、结构化】
                    {
                        ...
                        if (all_zones_.at(zone_sequence_.at(i - 1) - 1).zone_type == 1) // 【上一个区域：常规、结构化———当前区域：终点、结构化】
                        {
                            threadLogger_->info("【上一个区域：常规、结构化———当前区域：终点、结构化】");

                            if (end_point_to_pre_area) // 【上一个区域：常规、结构化———当前区域：终点、结构化、 规划终点靠近上一个区域】
                            {
                                threadLogger_->info("【上一个区域：常规、结构化———当前区域：终点、结构化、 规划终点靠近上一个区域】");
                            ...
                            {
                                threadLogger_->info("【上一个区域：常规、结构化———当前区域：终点、结构化、 规划终点远离上一个区域】");
                        ...
                        else // 【上一个区域：常规、非结构化———当前区域：终点、结构化】
                        {
                            ...
                            if (end_point_to_pre_area) // 【上一个区域：常规、非结构化———当前区域：终点、结构化、 规划终点靠近上一个区域】
                            {
                                threadLogger_->info("【上一个区域：常规、非结构化———当前区域：终点、结构化、 规划终点靠近上一个区域】");
                            ...
                            {
                                threadLogger_->info("【上一个区域：常规、非结构化———当前区域：终点、结构化、 规划终点远离上一个区域】");
 ...
 * @param [return] true: 规划成功；false：规划失败
 */
ErrorType Shenyan::PlanningCase6()
{
        ...
        else // 当前遍历到的区域为结构化区域，在planningcase6中，结构化区域只可能是中间区域
        {
            ...
            if (i + 1 != zone_sequence_.size() - 1) // 【当前区域：常规、结构化——下一个区域：常规】
            {
                ...
                if (has_start_hy)
                {
                    ...
                    global_path_.insert(global_path_.end(), temp_traj.begin() + Crop_start_Point, temp_traj.begin() + Crop_end_Point);
                }
                else
                {
 ...
 * @param [return]
 */
void Shenyan::SetMaxSpeed(std::vector<Trajectory_Point> &temp_path, float32 max_speed)
{
...
float32 Shenyan::CalNearestDistance(Single_Point point, Single_Border border_points)
{
 ...
 */
uint8 Shenyan::FindZoneId(Single_Point point, uint8 &zone_type)
{
    // LOG(INFO) << "FindZoneId";
#if 0
    ...
    if ((flag == true) && (task_type_ == static_cast<uint>(TaskType::GO_LOAD_POINT) || task_type_ == static_cast<uint>(TaskType::GO_UNLOAD_POINT)) && ((fabs(point.x - end_point_.x) < 1e-3) && (fabs(point.y - end_point_.y) < 1e-3) && (fabs(point.yaw - end_point_.yaw) < 1e-3)))
    {
    ...
    else if ((flag == true) && (task_type_ == static_cast<uint>(TaskType::GO_LOADAREA_WAITAREA) || task_type_ == static_cast<uint>(TaskType::GO_LOAD_TRANSITIONS_POINT) || task_type_ == static_cast<uint>(TaskType::GO_APART_LOADING_POINT) || task_type_ == static_cast<uint>(TaskType::GO_UNLOADAREA_WAITAREA)) &&
             ((fabs(point.x - start_point_.x) < 1e-3) && (fabs(point.y - start_point_.y) < 1e-3) && (fabs(point.yaw - start_point_.yaw) < 1e-3)))
    {
 ...
 */
bool Shenyan::PointInZone(Single_Point point, Single_Border border)
{
    ...
    for (int i = 0; i < border.border_points.size(); i++) // 遍历多边形每一个节点
    {
        ...
        p2 = border.border_points.at((i + 1) % border.border_points.size()); // p1是这个节点，p2是下一个节点，两点连线是多边形的一条边
                                                                             // 以下算法是用是先以y轴坐标来判断的
        if (p1.y == p2.y)
...
ErrorType Shenyan::FindReferencePath(Single_Point point, vector<Single_Trajectory> &trajs, uint32 &nearest_id, uint8 &path_id, bool flag, map<uint8, double> &v_id, double search_distance)
{
    v_id.clear();
    if (flag == true)
    {
    ...
    {
        threadLogger_->info("当前匹配轨迹ID:{},start:{},curr:{},end:{}", (int)trajs.at(i).id, (int)trajs.at(i).start_zone_id, (int)trajs.at(i).current_zone_id, (int)trajs.at(i).end_zone_id);

        if (start_node_ == end_node_)
        {
            if (FindSuitableReferenceLine(point, trajs.at(i), search_distance, lat_distance)) // 寻找大致合适的参考路径
            {
...
bool Shenyan::FindSuitableReferenceLine(Single_Point point, Single_Trajectory traj, double search_distance, double lat_dis)
{
    ...
    for (uint32 i = 0; i < traj.trajectory.size(); i++)
    {
        ...
        if (temp_distance2 < distance_min2 && ((fabs(point.yaw - temp_traj_point.yaw) / M_PI * 180) < 90 || (fabs(point.yaw - temp_traj_point.yaw) / M_PI * 180) > 270))
        {
    ...
    if (distance_min2 < search_distance * search_distance)
    {
        ...
        lat_distance = -(point.x - nearest_point.x) * sin(nearest_point.yaw) + (point.y - nearest_point.y) * cos(nearest_point.yaw);
        lat_dis = fabs(lat_distance);
        threadLogger_->info("find suitable referenceline");
        return true;
    }
    else
    {
 ...
 */
bool Shenyan::FindNearestPoint(Single_Point point, Single_Trajectory traj, float32 &distance, uint32 &nearest_id)
{
    ...
    else
    {
        ...
        threadLogger_->info("这条轨迹方向与待匹配点的朝向存在相符的片段，找到的最近点几何距离为：{},索引为：{}", distance, nearest_id);
    }
    return temp_bool;
}
 ...
 * @param [return] true：成功；false：失败
 */
bool Shenyan::FindPathId(uint8 s_id, uint8 c_id, uint8 e_id, vector<Single_Trajectory> &trajs, uint8 &path_id)
{
    bool temp = false;
    ...
    for (int i = 0; i < trajs.size(); i++)
    {
        ...
        if (s_id == temp_s_id && c_id == temp_c_id && e_id == temp_e_id) // 目前假定一个方向只有一个参考路径，找到一个满足要求的即可
        {
            path_id = i;
            temp = true;
            break;
 ...
 * @param [return] 无
 */
void Shenyan::Interpolate(float64 x0, float64 y0, float32 theta0,
...
void Shenyan::CalAcc(std::vector<Trajectory_Point> &temp_path)
{
    float32 last_acc;
 ...
 * 返回 temp_path 中的曲率信息
 */
void Shenyan::CalCurv(std::vector<Trajectory_Point> &temp_path)
{
#if 1
   ...
   for(uint32 i = 0; i<temp_path.size(); i++)
   {
     ...
     float32 delta_d = sqrt(pow(temp_path.at(id1).x-temp_path.at(id2).x,2)+pow(temp_path.at(id1).y-temp_path.at(id2).y,2));//两点之间的弦长
 ...
 * 返回 temp_path 中的位移信息
 */
void Shenyan::CalDistance(std::vector<Trajectory_Point> &temp_path)
{
 ...
 * 返回 无
 */
void Shenyan::Path_Interpolation(std::vector<Trajectory_Point> global_path_, std::vector<Trajectory_Point> &temp_path)
{
 ...
 * 返回
 */
void Shenyan::Path_Interpolation_Linear(std::vector<Trajectory_Point> global_path_, std::vector<Trajectory_Point> &temp_path)
{
    std::vector<Trajectory_Point> path_s;
    Trajectory_Point temp_point;
    uint32 index0 = 0;
...
void Shenyan::ChangeBorderType(Single_Point &point)
{
    // 若点在边界外，才对边界属性进行修改
    if (end_point_out_border_ == true)
    {
        m_vehicle_param_.border_change_range = 6;
        m_vehicle_param_.border_sample_inter = 20;
        float32 range = m_vehicle_param_.border_change_range; // 终点附近多大范围内需要改变边界点属性
        int N = m_vehicle_param_.border_sample_inter;         // 降采样参数 隔多少个点
    ...
    {
        m_vehicle_param_.border_change_range = 3;
        m_vehicle_param_.border_sample_inter = 20;
        float32 range = m_vehicle_param_.border_change_range; // 终点附近多大范围内需要改变边界点属性
        int N = m_vehicle_param_.border_sample_inter;         // 降采样参数 隔多少个点
        std::vector<Single_Border_Point> temp_border_points;
 ...
 * @param [return] true：成功；false：失败
 */
bool Shenyan::ApplyHibridAStar(Single_Point s_point,
                               ...
                               uint8 plan_rule_id)
{
    ...
    // 起始点、目标点结构转换
    HybridAStar ::Point temp_start_point(s_point.x, s_point.y, s_point.z, s_point.yaw, HybridAStar ::MotionDirection ::Forward);
    HybridAStar ::Point temp_end_point(e_point.x, e_point.y, e_point.z, e_point.yaw, HybridAStar ::MotionDirection ::Forward);
...
#endif
    // my_optimal_path_.threadLogger_ = threadLogger_;
    if (my_optimal_path_.SearchGlobalPath(temp_start_point, temp_end_point, road_outer_bound, road_inner_bound, m_vehicle_param_,
                                          final_path, planrule) == HybridAStar::PlanResult::Plan_OK)
    {
        ...
        if (plan_rule_id == 5 || plan_rule_id == 4)
        {
            ...
            //     {
            //         if (pow(fabs(traj.at(i).x - traj.at(j).x), 2) + pow(fabs(traj.at(i).y - traj.at(j).y), 2) < 0.8 * 0.8)
            //         {
...
tar_start_end Shenyan::ParseInputInfo(char *str)
{
    ...
    if (doc.HasMember("m_Veh_Param"))
    {
        ...
        if (val.IsObject())
        {
            ...
            //            veh_start_end.m_Veh_Param.max_steering = atan(veh_start_end.m_Veh_Param.wheel_base / veh_start_end.m_Veh_Param.front_radious) / M_PI * 180;
            ...
            if (val.HasMember("light_narrow_road_speed_limit"))
                veh_start_end.m_Veh_Param.light_narrow_road_speed_limit = val["light_narrow_road_speed_limit"].GetFloat();
            else
            {
            ...
            if (val.HasMember("light_slope_road_speed_limit"))
                veh_start_end.m_Veh_Param.light_slope_road_speed_limit = val["light_slope_road_speed_limit"].GetFloat();
            else
            {
            ...
            if (val.HasMember("light_intersection_road_speed_limit"))
                veh_start_end.m_Veh_Param.light_intersection_road_speed_limit = val["light_intersection_road_speed_limit"].GetFloat();
            else
            {
            ...
            if (val.HasMember("light_bumpy_road_speed_limit"))
                veh_start_end.m_Veh_Param.light_bumpy_road_speed_limit = val["light_bumpy_road_speed_limit"].GetFloat();
            else
            {
            ...
            if (val.HasMember("light_regular_road_speed_limit"))
                veh_start_end.m_Veh_Param.light_regular_road_speed_limit = val["light_regular_road_speed_limit"].GetFloat();
            else
            {
            ...
            if (val.HasMember("heavy_narrow_road_speed_limit"))
                veh_start_end.m_Veh_Param.heavy_narrow_road_speed_limit = val["heavy_narrow_road_speed_limit"].GetFloat();
            else
            {
            ...
            {
                veh_start_end.m_Veh_Param.heavy_slope_road_speed_limit = val["heavy_slope_road_speed_limit"].GetFloat();
                cout << "find heavy_slope_road_speed_limit " << veh_start_end.m_Veh_Param.heavy_slope_road_speed_limit << endl;
            }
            else
            {
            ...
            if (val.HasMember("heavy_intersection_road_speed_limit"))
                veh_start_end.m_Veh_Param.heavy_intersection_road_speed_limit = val["heavy_intersection_road_speed_limit"].GetFloat();
            else
            {
            ...
            if (val.HasMember("heavy_bumpy_road_speed_limit"))
                veh_start_end.m_Veh_Param.heavy_bumpy_road_speed_limit = val["heavy_bumpy_road_speed_limit"].GetFloat();
            else
            {
            ...
            if (val.HasMember("heavy_regular_road_speed_limit"))
                veh_start_end.m_Veh_Param.heavy_regular_road_speed_limit = val["heavy_regular_road_speed_limit"].GetFloat();
            else
            {
            ...
            if (val.HasMember("multi_section_speed_limit_enable"))
                veh_start_end.m_Veh_Param.multi_section_speed_limit_enable = val["multi_section_speed_limit_enable"].GetUint();
            else
            {
...
bool Shenyan::ProgressiveHybirdAStar(Single_Point &input_point, uint8 input_node, bool search_direction, std::vector<Trajectory_Point> &input_trajectory, uint32 search_start, int32 &search_index, std::vector<Trajectory_Point> &result_trajectory, uint8 rule_id)
{
    ...
    if (search_direction == true) // 顺着参考线进行搜索，这种case为找拼接终点
    {
        ...
        for (int32 i = search_start; i < input_trajectory.size(); i += 5)
        {
            ...
            if (ApplyHibridAStarWithTime(input_point, temp_end, input_node, result_trajectory, rule_id, time_threshold)) // rule_id:5，只前进
            {
                search_index = i;
                success_flag = true;
                break;
    ...
    { // 逆着参考线进行搜索，这种case为找拼接起点
        ...
        for (int32 i = search_start; i > 0; i -= 5)
        {
            ...
            if (counter > 4)
                time_threshold = 0.8 * 1000 * 1000;
            if (ApplyHibridAStarWithTime(temp_start, input_point, input_node, result_trajectory, rule_id, time_threshold)) // rule_id:5，只前进
            {
                search_index = i;
                success_flag = true;
                break;
...
bool Shenyan::ApplyHibridAStarWithTime(Single_Point s_point,
                                       Single_Point e_point,
                                       uint8 temp_zone_id,
                                       vector<Trajectory_Point> &traj,
                                       uint8 plan_rule_id,
                                       int64 time_threshold)
{
    ...
    my_optimal_path_.threadLogger_ = threadLogger_;
#endif
    if (my_optimal_path_.SearchGlobalPath1(temp_start_point, temp_end_point, road_outer_bound, road_inner_bound, m_vehicle_param_,
...
bool Shenyan::DetermineIfOrNoHybirdAStar(Single_Point &input_point, std::vector<Trajectory_Point> &traj, uint32 nearest_point_index, float &lat_distance_)
{
    ...
    float64 lat_distance; // 起点与参考轨迹最近点的横向距离
    lat_distance = -(input_point.x - nearest_point.x) * sin(nearest_point.yaw) + (input_point.y - nearest_point.y) * cos(nearest_point.yaw);
    lat_distance_ = fabs(lat_distance);
    threadLogger_->info("lat_distance:{}", lat_distance_);
    if (pow(nearest_point.x - input_point.x, 2) + pow(nearest_point.y - input_point.y, 2) > pow(fabs(lat_distance) + 1.5, 2))
    {
        threadLogger_->info("几何距离：{}", sqrt(pow(nearest_point.x - input_point.x, 2) + pow(nearest_point.y - input_point.y, 2)));
        threadLogger_->info("与匹配点纵向距离太远，需要进行hybird A*拟合");
    ...
    if (fabs(lat_distance) < (m_vehicle_param_.lat_min_distance_struct + eps) && fabs(delta_angle) < (M_PI / 18 + eps))
    {
        return false;
    }
    else
    {
        if (fabs(lat_distance) > (m_vehicle_param_.lat_min_distance_struct + eps))
        {
...
bool Shenyan::removeDuplicates(vector<Trajectory_Point> &input, std::vector<Trajectory_Point> &result)
{
    result.clear();
...
// 成功：true，失败：false
vector<Trajectory_Point> Shenyan::RandomOffsetWithoutCuravture(vector<Trajectory_Point> &path)
{
        ...
        if (path_.front().direction == 0)
        {
            ...
            for (int i = 0; i < forward_section.size(); i++)
            {
                ...
                for (m = start; m <= end; m = m + 1)
                {
                    ...
                    if (end - start > 250)
                    {
                        off_set = CalculateOffSetWithoutCuravture(a, sum + 1, weight); // off_set的大小决定偏移幅度，正负决定偏移方向
                    }
                    else // 如果改段路太短，不进行偏移
                    {
                        off_set = 0.0;
                ...
                {
                    result_path.insert(result_path.end(), path_.begin() + reverse_section.at(j).first, path_.begin() + reverse_section.at(j).second + 1);
                    j++;
                }
            }
        }
...
float Shenyan::CalculateOffSetWithoutCuravture(int index, int sum, float weight)
{
    // threadLogger_->info("weight:{}", weight);
    float max_l = m_vehicle_param_.max_l;
    float L = max_l * weight; // 控制默认偏移量
    L = L * WeightFunction(index, sum);
    return L;
}
void Shenyan::CalculateCubicSplineCurve(const std::vector<Trajectory_Point> points, std::vector<Trajectory_Point> &cubicspline_path)
{
...
bool Shenyan::PathCollisionCheck(std::vector<Trajectory_Point> Path)
{
    // 确定所有的区域的外边界和内边界
    ...
    midpoint_.z = 0;
    threadLogger_->info("start_point_.x:{} ,start_point_.y:{}, end_point_.x:{},end_point_.y:{}, midpoint_.x:{}, midpoint_.y:{}", start_point_.x, start_point_.y, end_point_.x, end_point_.y, midpoint_.x, midpoint_.y);
...
void Shenyan::CalculateStation(const std::vector<float64> &xs, const std::vector<float64> &ys)
{
    float64 cum = 0.0;
    s_.clear();
    s_.push_back(cum);
...
bool Shenyan::CheckCurvature(std::vector<Trajectory_Point> &tra)
{
...
float Shenyan::WeightFunction(int k, int sum)
{
    // int sita = m_vehicle_param_.sita;
    int sita = 200;
    int x;
    if (k < sum / 2)
    {
    ...
    // return 1.0 / (1 + pow(m_vehicle_param_.base, (-(x - sita))));
    return 1.0 / (1 + pow(1.1, (-(x - sita))));
}

bool Shenyan::doesTrajectorySelfIntersect(Path &path)
{
    // 通过判断yaw的变化了分析是否画圈
...
// bool Shenyan::CheckDirection(Path &input_path)
// {
//     double angle;
//     for (int i = 0; i < input_path.size() - 1; i++)
//     {
//         angle = atan((input_path.at(i + 1).y - input_path.at(i).y) / (input_path.at(i + 1).x - input_path.at(i).x));
//         if (angle)
//     }
// }

bool Shenyan::checkPathFracture(std::vector<Trajectory_Point> &input_path)
{
    double dis = 0;
    for (int i = 0; i < input_path.size() - 1; i++)
    {
        dis = pow(input_path.at(i).x - input_path.at(i + 1).x, 2) + pow(input_path.at(i).y - input_path.at(i + 1).y, 2);
        if (dis > 9)
        {
...
bool Shenyan::OverSpeedCheck(std::vector<Trajectory_Point> &input_path)
{
    bool flag = false;
    for (int i = 0; i < input_path.size() - 1; i++)
    {
        ...
        {
            cout << "i:" << i << "speed:" << input_path.at(i).speed << "speed_limit:" << input_path.at(i).speed_limit << endl;
            threadLogger_->info("i:{}  speed:{}   speed_limit:{}", i, input_path.at(i).speed, input_path.at(i).speed_limit);
            flag = true;
        }
    }
    if (flag == true)
