...
bool GetMap(char* parea) {
    {
        ...
        cout << "GlobalPathPlanning-IDS_Global_Planning_version: G_V1.4.0.20250103_beta" << endl;
        cout << "规划库版本号:G_V1.4.0.20250103_beta" << endl;
        ...
        if (GlobalPlanning::Parser::GetMap(parea)) {
            ...
            ss << std::put_time(std::localtime(&timestamp), "%Y-%m-%d-%H-%M-%S");
            timeStr = ss.str();
            record << timeStr << "， 地图更新成功***************************G_V1.4.0.20250103_beta" << endl;
            record.close();
            return true;
        }
        else {
            ...
            ss << std::put_time(std::localtime(&timestamp), "%Y-%m-%d-%H-%M-%S");
            timeStr = ss.str();
            record << timeStr << " ，地图更新失败***************************G_V1.4.0.20250103_beta" << endl;
            record.close();
            return false;
        }
    }
...
char* GlobalPathPlanning(char* point_veh_start_end) {
    ...
    ss << std::put_time(std::localtime(&timestamp), "%Y-%m-%d-%H-%M-%S");
    std::string timeStr = ss.str();
    cout << "**********************欢迎光临后台全局规划库,版本号:G_V1.4.0.20250103_beta************************************" << timeStr << endl;
    ...
    planning.vehicle_code_   = vehicle_code;
    {
        ...
        record << timeStr << " ，收到规划请求，请求号：" << veh_start_end.my_key << "，车辆编号：" << vehicle_code << "    规划库版本号:G_V1.4.0.20250103_beta" << endl;
        record.close();
    }
    ...
    planning.threadLogger_->info("GlobalPathPlanning-IDS_Global_Planning_version: G_V1.4.0.20250103_beta");
