 ...
 * ****************************************/
#include "../include/global_speed_planning_test.h"
 ...
 *return
 */
void GlobalSpeedPlanning ::InitSpeedParam(vehicle_param m_veh_param)
{
 ...
 */
bool GlobalSpeedPlanning ::SpeedPlanning(std ::vector<Trajectory_Point> &trajectory, const vehicle_param m_veh_param, const int32 total_time,
                                         const int32 departure_time, uint8 mode)
{
    ...
    // /* 参数检查，如果路点小于3个，则直接退出 */
    // if (trajectory_points.size() < 3)
    // {
    //     threadLogger_->error("...The trajectory_point.size()<3, the global speed planning failed...");
 ...
 *return  true: 规划成功；false：规划失败
 */
bool GlobalSpeedPlanning ::PlanCase1(const int32 total_time, const int32 departure_time)
{
    float32 sum_time = 0;
    for (uint8 i = 0; i < key_points.size(); i++)
    {
        /* 得到梯形速度曲线 */
        if (!TrapezoidalSpeedPlanning(i))
        {
            threadLogger_->error("...TrapezoidalSpeedPlanning failed in {} 端路径...", (float32)i);
    ...
    float32 delta_time = total_time - sum_time;
    while (fabs(sum_time - total_time) > 5)
    {
        global_speeds.clear();
        opti_global_speeds.clear();
        float32 kDeltaSpeed_adap = kDeltaSpeed * (total_time - sum_time) / delta_time;
    ...
    for (uint8 i = 0; i < key_points.size(); i++)
    {
        ...
        {
            for (uint32 j = 0; j < trajectory_fragments.at(i).size(); j++)
 ...
 *return  true: 规划成功；false：规划失败
 */
bool GlobalSpeedPlanning ::PlanCase0(const int32 departure_time)
{
    threadLogger_->info("key_points.size() = {}", key_points.size());

    for (uint8 i = 0; i < key_points.size(); i++)
    {
        threadLogger_->info(" ...plan for the {} the segment...", (float32)(i + 1));
 ...
 * @param [return]
 */
void GlobalSpeedPlanning ::ReducedMaximumSpeedAve(float32 delta_speed)
{
    float32 max_speed = 0;
    for (uint32 i = 0; i < key_points.size(); i++)
    {
        for (uint32 j = 0; j < key_points.at(i).size(); j++)
        {
    ...
    for (uint32 i = 0; i < key_points.size(); i++)
    {
        if (0 == key_points.at(i).at(0).direction)
        {
            for (uint32 j = 0; j < key_points.at(i).size(); j++)
            {
                float32 temp_speed1 = key_points.at(i).at(j).speed_limit_left - key_points.at(i).at(j).speed_limit_left * delta_speed / max_speed;
                float32 temp_speed2 = key_points.at(i).at(j).speed_limit_right - key_points.at(i).at(j).speed_limit_right * delta_speed / max_speed;
...
void GlobalSpeedPlanning ::ReducedMaximumSpeed()
{
    ...
    for (uint32 i = 0; i < key_points.size(); i++)
    {
        for (uint32 j = 0; j < key_points.at(i).size() - 1; j++)
        {
            if (key_points.at(i).at(j).speed_limit_right > max_speed)
            {
                max_speed = key_points.at(i).at(j).speed_limit_right;
    ...
    float32 temp_speed1 = key_points.at(id1).at(id2).speed_limit_right - kDeltaSpeed;
    float32 temp_speed2 = key_points.at(id1).at(id2 + 1).speed_limit_left - kDeltaSpeed;
 ...
 * @return 无
 */
bool GlobalSpeedPlanning ::TrapezoidalSpeedPlanning(uint8 num)
{
    std ::vector<SparseSpeedPoint> temp_sparsespeedpoints;
    std ::vector<SparseSpeedPoint> temp_sparsespeedpoints_all;
    std ::vector<KeyPoint> temp_keypoint = key_points.at(num);
    ...
    float32 last_speed;            // PlanForSingleSegment()函数规划后，车辆能够达到的速度，最后一个点的速度
    if (2 == temp_keypoint.size()) // 若只有两个关键点，后面合成一条路径的时候再来根据方向，改变速度符号
    {
        ...
        {
            threadLogger_->error("...Plan for the segment {} failed...", (float32)num);
        ...
        //   std :: threadLogger_->info("...last_speed is "<<last_speed<<" ..."<<endl;
    ...
    else if (2 < temp_keypoint.size()) // 若大于两个关键点
    {
        ...
        /*循环遍历关键点集*/
        float32 last_speed = temp_keypoint.at(0).speed_limit_left;
        for (uint32 i = 0; i < temp_keypoint.size() - 1; i++)
        {
            keypoint1 = temp_keypoint.at(i);
            keypoint2 = temp_keypoint.at(i + 1);
            //         std :: threadLogger_->info("...keypoint1.index= "<<keypoint1.index<<"..."<<endl;
            //         std :: threadLogger_->info("...keypoint2.index= "<<keypoint2.index<<"..."<<endl;
        ...
        //       threadLogger_->info("*****global_speeds.back().back().speed = " << global_speeds.back().back().speed << "\n";
        return true;
    }
    else
    {
        threadLogger_->error("...Plan for the segment {}  failed, the keypoint number is less than 2...", (float32)num);
...
bool GlobalSpeedPlanning ::KeyPointsDecelerationCheck()
{
    ...
    float32 v1, v2;
    KeyPoint keypoint1, keypoint2;
    std ::vector<Trajectory_Point> temp_traj;
    for (int i = 0; i < key_points.size(); i++)
    {
        ...
        for (int j = 0; j < key_points.at(i).size() - 1; j++)
        {
            ...
            threadLogger_->info("第{}段的第{}个关键点的索引 {},左限速 {},右限速 {}", (float32)(i + 1), j + 1, key_points.at(i).at(j).index, key_points.at(i).at(j).speed_limit_left, key_points.at(i).at(j).speed_limit_right);

            threadLogger_->info("第{}段的第{}个关键点的索引 {},左限速 {},右限速 {}", (float32)(i + 1), j + 2, key_points.at(i).at(j + 1).index, key_points.at(i).at(j + 1).speed_limit_left, key_points.at(i).at(j + 1).speed_limit_right);
            ...
            if (v1 > v2)
            {
                ...
                float32 s_total = temp_traj.at(keypoint2.index).distance - temp_traj.at(keypoint1.index).distance; // 该路段的路径长度(可能为弧线)
                float32 s_min = (pow(v2, 2) - pow(v1, 2)) / (2 * kMinAcceleration);                                // v1_left加速到v1_right所需的最小欧式距离
...
//     std ::vector<std ::vector<KeyPoint>> temp_key_points;
//     std ::vector<KeyPoint> temp_point;
//     std ::vector<Trajectory_Point> temp_traj;
...
//                 for (int j = 0; j < key_points.at(i).size() - 1; j++)
//                 {
//                     float32 v1_left, v1_right, v2_left, v2_right;
//                     KeyPoint keypoint1, keypoint2;
//                     keypoint1 = key_points.at(i).at(j);
//                     keypoint2 = key_points.at(i).at(j + 1);
...
//                         float32 s_total = temp_traj.at(keypoint2.index).distance - temp_traj.at(keypoint1.index).distance; // 该路段的路径长度(可能为弧线)
//                         float32 s_min = (pow(v1_right, 2) - pow(v1_left, 2)) / (2 * kMaxAcceleration);                     // v1_left加速到v1_right所需的最小欧式距离
//                         threadLogger_->info("s_total = {}", s_total);
//                         threadLogger_->info("s_min ={} ", s_min);
...
//                             if (j != 0)
//                             {
//                                 key_points.at(i).at(j + 1).speed_limit_left = v1_left; // 每一次的赋值,都是为了在下一个循环中使用当前的修改
//                                 flag = true;
//                                 continue;
//                             }
//                         }
...
//                         if ((v2_right - 0.0) < eps)
//                         {
//                             float32 s_min = (pow(v2_right, 2) - pow(v1_right, 2)) / (2 * kMinAcceleration); // v1_left减速到v1_right所需的最小欧式距离
//                             threadLogger_->info("s_total = {}", s_total);
//                             threadLogger_->info("s_min = {}", s_min);
...
//                         else
//                         {
//                             float32 s_min = (pow(v2_right, 2) - pow(v1_left, 2)) / (2 * kMinAcceleration); // v1_left减速到v1_right所需的最小欧式距离
//                             threadLogger_->info("s_total ={} ", s_total);
//                             threadLogger_->info("s_min = {}", s_min);
//                             if (((s_total + eps) < s_min))
//                             {
...
//                         float32 s_total = temp_traj.at(keypoint2.index).distance - temp_traj.at(keypoint1.index).distance; // 该路段的路径长度(可能为弧线)
//                         float32 s_min = (pow(v1_right, 2) - pow(v1_left, 2)) / (2 * kMinAcceleration);                     // v1_left减速到v1_right所需的最小欧式距离
//                         threadLogger_->info("s_total = {} ", s_total);
//                         threadLogger_->info("s_min = {}", s_min);
//                         if ((s_total + eps) < s_min)
//                         {
//                             threadLogger_->info("~case6~");
//                             float32 delta_front_distance, delta_back_distance;
...
//                                     // keypoint1 前的距离比较长
//                                     threadLogger_->info("key_points.at(i).at(j).index = {}", key_points.at(i).at(j).index);
//                                     uint temp_index = floor(delta_front_distance / delta_s) / 40;
//                                     // temp_index = 0;
...
//                                     key_points.at(i).at(j).distance = temp_traj.at(key_points.at(i).at(j).index).distance;
//                                     threadLogger_->info("key_points.at(i).at(j).index = {}", key_points.at(i).at(j).index);
//                                     threadLogger_->info("key_points.at(i).at(j).left = {}", key_points.at(i).at(j).speed_limit_left);
//                                     threadLogger_->info("key_points.at(i).at(j).right = {}", key_points.at(i).at(j).speed_limit_right);
//                                 }
...
//                                 {
//                                     // keypoint2 后的距离比较长
//                                     threadLogger_->info("key_points.at(i).at(j+1).index = {}", key_points.at(i).at(j + 1).index);
//                                     uint temp_index = floor(delta_back_distance / delta_s) / 40;
//                                     // temp_index = 0;
...
//                                     {
//                                         int32 temp_index1 = temp_traj.size() - 1;
//                                         key_points.at(i).at(j + 1).index = temp_index1;
//                                         key_points.at(i).at(j + 1).distance = temp_traj.at(key_points.at(i).at(j + 1).index).distance;

//                                         // key_points.at(i).at(j + 1).distance += (temp_index1 - key_points.at(i).at(j + 1).index) * delta_s;
//                                         // key_points.at(i).at(j + 1).index = temp_index1;
//                                     }
//                                     else
//                                     {
...
//                             float32 s_total = temp_traj.at(keypoint2.index).distance - temp_traj.at(keypoint1.index).distance; // 该路段的路径长度(可能为弧线)
//                             float32 s_min = (pow(v2_left, 2) - pow(v1_left, 2)) / (2 * kMaxAcceleration);                      // v1_left减速到v1_right所需的最小欧式距离
//                             threadLogger_->info("s_total = {} ", s_total);
//                             threadLogger_->info("s_min = {}", s_min);
...
//         {
//             file_out << key_points.at(index).at(jndex).index << " " << key_points.at(index).at(jndex).speed_limit_left << " " << key_points.at(index).at(jndex).speed_limit_right << endl;
//         }
//     }
//     file_out.close();
        ...
        for (int i = 0; i < key_points.size(); i++)
        {
            ...
            else
            {
                ...
                for (int j = 0; j < key_points.at(i).size() - 1; j++)
                {
                    ...
                    {
                        // 判断距离 若距离符合 将中速降低为最小速度
                        float32 s_total = temp_traj.at(keypoint2.index).distance - temp_traj.at(keypoint1.index).distance;
                        float32 s_min = (pow(v1_right, 2) - pow(v1_left, 2)) / (2 * kMaxAcceleration) + (pow(v2_right, 2) - pow(v1_right, 2)) / (2 * kMinAcceleration) + 10; // v1_left加速到v1_right，然后再由v1_right减速到v2_right所需要的极限距离
                        threadLogger_->info("s_total = {}", s_total);
 ...
 * @param [return]
 */
bool GlobalSpeedPlanning ::PlanForSingleSegment(uint8 num,
                                                KeyPoint keypoint1,
                                                KeyPoint keypoint2,
                                                std ::vector<SparseSpeedPoint> &temp_sparsespeedpoints,
                                                float32 &last_speed)
{
    ...
    std ::vector<Trajectory_Point> temp_traj = trajectory_fragments.at(num);
    SparseSpeedPoint temp_sparsepoint;
    float32 v0, v1, v2;
    ...
    if (fabs(v0 - v1) < 0.01 && v2 < v1)
    {
        ...
        float32 s_total = temp_traj.at(keypoint2.index).distance - temp_traj.at(keypoint1.index).distance; // 该路段的路径长度
        float32 s_min = (pow(v2, 2) - pow(v1, 2)) / (2 * kMinAcceleration);                                // 减速到v2所需的最小距离
        ...
        {
            float32 s_temp, s_d;
            float32 s_1 = s_total - s_min; // 匀速截止的距离
            float32 s_0 = temp_traj.at(keypoint1.index).distance;
            for (uint32 i = keypoint1.index; i < keypoint2.index; i += kDiscreteNumber)
            {
    ...
    if (v0 < v1 && v2 >= v1)
    {
        ...
        float32 s_total = temp_traj.at(keypoint2.index).distance - temp_traj.at(keypoint1.index).distance; // 该路段的路径长度
        float32 s_min = (pow(v1, 2) - pow(v0, 2)) / (2 * kMaxAcceleration);                                // 加速到v2所需的最小距离
        float32 s_temp;
        float32 s_0 = temp_traj.at(keypoint1.index).distance;
        for (uint32 i = keypoint1.index; i < keypoint2.index; i += kDiscreteNumber)
        {
   ...
   需要加速，再减速 */
    if (v0 < v1 && v2 < v1 && v2 <= v0)
    {
        ...
        float32 s_total = temp_traj.at(keypoint2.index).distance - temp_traj.at(keypoint1.index).distance; // 该路段的路径长度
        float32 s_min = (pow(v2, 2) - pow(v0, 2)) / (2 * kMinAcceleration);                                // 减速到v2所需的最小距离
        if (s_total < s_min)
        {
            threadLogger_->error("...Case 4: s_total ={}  s_min ={}  s_total < s_min...", s_total, s_min);
        ...
        float32 s_acc = (pow(v1, 2) - pow(v0, 2)) / (2 * kMaxAcceleration);
        float32 s_dec = (pow(v2, 2) - pow(v1, 2)) / (2 * kMinAcceleration);
        if (s_acc + s_dec < s_total) // 先加速到最大速度，再减速到v2
        {
            ...
            for (uint32 i = keypoint1.index; i < keypoint2.index; i += kDiscreteNumber)
            {
                ...
                {
                    float32 s_d = s_temp - (s_total - s_dec); // 减速距离
                    temp_sparsepoint.index = i;
        ...
        {
            float32 s_as = (2 * kMinAcceleration * s_total - pow(v2, 2) + pow(v0, 2)) / (2 * (kMinAcceleration - kMaxAcceleration)); // 加速的距离
            float32 v_max = sqrt((2 * kMinAcceleration * kMaxAcceleration * s_total + kMinAcceleration * pow(v0, 2) - kMaxAcceleration * pow(v2, 2)) / (kMinAcceleration - kMaxAcceleration));
            float32 s_temp;
            float32 s_0 = temp_traj.at(keypoint1.index).distance;
            for (uint32 i = keypoint1.index; i < keypoint2.index; i += kDiscreteNumber)
            {
    ...
    if (v0 < v1 && v2 < v1 && v2 > v0)
    {
        ...
        float32 s_total = temp_traj.at(keypoint2.index).distance - temp_traj.at(keypoint1.index).distance; // 该路段的路径长度
        float32 s_min = (pow(v2, 2) - pow(v0, 2)) / (2 * kMaxAcceleration);                                // 加速到v2所需的最小距离
        float32 s_acc = (pow(v1, 2) - pow(v0, 2)) / (2 * kMaxAcceleration);
        float32 s_dec = (pow(v2, 2) - pow(v1, 2)) / (2 * kMinAcceleration);
        if (s_acc + s_dec < s_total) // 先加速到最大速度，再减速到v2
        {
...
bool GlobalSpeedPlanning ::SplitPath()
{
    ...
    end_id = 0;
    std ::vector<Trajectory_Point> temp_traj;
    for (uint32 i = 0; i < trajectory_points.size() - 1; i++)
    {
        if (trajectory_points.at(i).direction != trajectory_points.at(i + 1).direction || trajectory_points.at(i).attribute == 6 || trajectory_points.at(i).attribute == 7)
        {
            ...
            // {
            //     threadLogger_->info("...The trajectory for one direction is too short 1, the trajectory is not feasible...");
        ...
        if (i == trajectory_points.size() - 2) //  最后一段
        {
            // if (i - start_id < 2 * kDiscreteNumber) //  如果单段路径路点数量太少，认为路径不合理
            // {
            //     threadLogger_->info("...The trajectory for one direction is too short 2, the trajectory is not feasible...");
...
bool GlobalSpeedPlanning ::GetKeypoint()
{
    ...
    {
        for (uint32 i = 0; i < trajectory_fragments.size(); i++)
        {
            temp_traj.clear();
            temp_traj = trajectory_fragments.at(i);
            if (1 == temp_traj.at(0).direction) // 如果是倒车，暂时按照最简单的加速、匀速、加速的模式进行速度规划，全段最大速度提前设定为 reverse_speed
            {
            ...
            else if (0 == temp_traj.at(0).direction) // 如果是前行，可能有多个不同的限速，关键点数量大于等于两个
            {
                ...
                for (uint32 j = 0; j < temp_traj.size() - 1; j++)
                {
                    if (temp_traj.at(j).speed_limit != temp_traj.at(j + 1).speed_limit) // 最大速度改变处设置一个关键点
                    {
                        temp_keypoint.Set(j, temp_traj.at(j).distance, 0, temp_traj.at(j).speed_limit, temp_traj.at(j + 1).speed_limit); // 给关键点赋值
                        temp_keypoints.emplace_back(temp_keypoint);
                    }
                    if (temp_traj.size() - 2 == j) // 最后一个路径点也作为关键点
                    {
                        temp_keypoint.Set(j + 1, temp_traj.at(j + 1).distance, 0, temp_traj.at(j + 1).speed_limit, 0.0); // 给关键点赋值

                        temp_keypoints.emplace_back(temp_keypoint);
                    }
                }
                key_points.emplace_back(temp_keypoints);
                //               threadLogger_->info("key_points.back().back().index =" << key_points.back().back().index << "\n";
                //               threadLogger_->info("key_points.back().back().direction =" << static_cast<int>(key_points.back().back().direction) << "\n";
                //               threadLogger_->info("key_points.back().back().speed_limit_left =" << key_points.back().back().speed_limit_left << "\n";
                //               threadLogger_->info("key_points.back().back().speed_limit_right =" << key_points.back().back().speed_limit_right << "\n";
            }
        }
    }
    else
    ...
    for (int i = 0; i < key_points.size(); i++)
    {
        threadLogger_->info("第{}段有{}关键点", (float32)(i + 1), key_points.at(i).size());
 ...
 * @param  num：trajectory_fragments中的第num条
 */
void GlobalSpeedPlanning ::SpeedCurveSmooth(uint8 num)
{
    ...
    while (iterations++ < 100)
    {
        ...
        for (uint32 i = 1; i < global_speeds.at(num).size() - 1; i++)
        {
            float32 v0 = temp_opti_global_speed.at(i - 1).speed;
            float32 v1 = temp_opti_global_speed.at(i).speed;
            float32 v2 = temp_opti_global_speed.at(i + 1).speed;
            float32 vo = global_speeds.at(num).at(i).speed;

            float32 gradient_error = kErrorTerm * (vo - v1);
            float32 gradient_smooth = kSmoothnessTerm * (v0 + v2 - 2 * v1);
 ...
 * @brief: 计算当前速度曲线总用时函数
 */
float32 GlobalSpeedPlanning ::GetCurrentTotalTime()
{
    float32 total_t = 0;
    // 遍历稀疏速度曲线
    for (uint8 i = 0; i < opti_global_speeds.size(); i++)
    {
        std::vector<SparseSpeedPoint> temp_opt_speeds = opti_global_speeds.at(i);
        std ::vector<Trajectory_Point> temp_traj = trajectory_fragments.at(i);
        for (uint32 j = 1; j < temp_opt_speeds.size(); j++)
        {
            // 计算出相邻两个插值点之间的距离。
            float32 delta_s = temp_traj.at(temp_opt_speeds.at(j).index).distance -
                              temp_traj.at(temp_opt_speeds.at(j - 1).index).distance;
            // 累加相邻两个插值点之间的时间，得到总用时间
            float32 ave_speed = (temp_opt_speeds.at(j).speed + temp_opt_speeds.at(j - 1).speed) / 2;
 ...
 * @param  num：trajectory_fragments中的第num条
 */
void GlobalSpeedPlanning ::SpeedCurveInterpolation(uint8 num)
{
    std ::vector<SparseSpeedPoint> temp_opti_global_speed = opti_global_speeds.at(num);
    std ::vector<Trajectory_Point> temp_traj = trajectory_fragments.at(num);
    // for (int i = 0; i < temp_traj.size(); i++)
    // {
    //     if (temp_traj.at(i).attribute == 6)
    //         threadLogger_->info("**********************find attribute==6**********************" );
    //     threadLogger_->info("i:" << i << " attriburte:" << (int)temp_traj.at(i).attribute << "  distance:" << temp_traj.at(i).distance );
    // }
    ...
    // 遍历稀疏速度曲线
    for (uint32 i = 1; i < temp_opti_global_speed.size(); i++)
    {
        // 计算出相邻两个插值点之间的距离。
        float32 total_s = temp_traj.at(temp_opti_global_speed.at(i).index).distance -
                          temp_traj.at(temp_opti_global_speed.at(i - 1).index).distance;
        // 计算当前两稀疏速度值之间的加速度
        float32 a = (pow(temp_opti_global_speed.at(i).speed, 2) - pow(temp_opti_global_speed.at(i - 1).speed, 2)) / (2 * total_s);
        float32 delta_s = 0;
        // 计算需要插值的路点数，采用匀加速模型进行速度插值，计算得到每个插值点的速度。
        for (int32 j = temp_opti_global_speed.at(i - 1).index; j < temp_opti_global_speed.at(i).index; j++)
        {
            float32 temp_speed = sqrt(pow(temp_opti_global_speed.at(i - 1).speed, 2) + 2 * a * delta_s);
            temp_traj.at(j - temp_opti_global_speed.at(0).index).speed = temp_speed;
            //           threadLogger_->info("temp_traj.at(" << j-temp_opti_global_speed.at(0).index << ").speed = " << temp_traj.at(j-temp_opti_global_speed.at(0).index).speed << "\n";
            delta_s += temp_traj.at(j + 1).distance - temp_traj.at(j).distance;
            //           std :: cout <<"...a = ..."<<a<<"\n";
            //           std :: cout <<"...temp_speed = ..."<<temp_speed<<"\n";
            //           std :: cout <<"...(j-temp_opti_global_speed.at(i - 1).index) = ..."<<(j-temp_opti_global_speed.at(0).index)<<"\n";
...
void GlobalSpeedPlanning ::AddTimeInformation(const int32 &start_time_)
{
    float64 t = 0.0 + start_time_;
    // 遍历插值后的速度曲线
    for (uint32 i = 0; i < trajectory_fragments.size(); i++)
    {
        trajectory_fragments.at(i).at(0).t = t;
        for (uint32 j = 1; j < trajectory_fragments.at(i).size(); j++)
        {
            // 计算出相邻两个插值点之间的距离。
            float32 delta_s = trajectory_fragments.at(i).at(j).distance - trajectory_fragments.at(i).at(j - 1).distance;
            // 累加相邻两个插值点之间的时间。
            float32 ave_speed = (trajectory_fragments.at(i).at(j - 1).speed + trajectory_fragments.at(i).at(j).speed) / 2;
            ave_speed = ave_speed > 0 ? ave_speed : 0.01;
            t += delta_s / ave_speed;
