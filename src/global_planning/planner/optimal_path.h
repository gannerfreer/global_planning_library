...
namespace HybridAStar
{
    ...
    {
        uint64 id;                 // id信息
        uint64 parent_id;          // 父节点id
        float64 x;                 // x坐标
        float64 y;                 // y坐标
        float64 z;                 // z坐标
        float64 angle;             // 角度,rad
    ...
    public:
         ...
         * 返回规划结果
         */
        PlanResult SearchGlobalPath(const Point start,
                                    const Point end,
        ...
        PlanResult SearchGlobalPath1(const Point start,
                                     const Point end,
                                     const Bound &road_bound,
                                     const Bound &obstacle_bound,
                                     const vehicle_param m_vehicle_param,
    ...
    private:
         ...
         * 返回规划结果
         */
        PlanResult AStarPath(Path &path, int64 timeThreshold);
         ...
         * @return 返回说明：无
         */
        void FindExpandVertex(const Vertex3D &current_point, uint64 &time_expand, uint64 &time_expand_another);
         ...
         * @return 返回说明：无
         */
        void VehDynam(const Vertex3D &start,
         ...
         */
        float32 AStarSearch2D(Node2D &start, Node2D &goal);
         ...
         */
        inline IntCoordinate Hash2Coordinate(uint64 hash)
        {
            IntCoordinate point;
            point.x = hash >> 16;
            point.y = hash & 0xFFFF;
         ...
         * @return 返回说明：对应哈希值
         */
        inline uint64 Vertex2Hash(Vertex3D v)
        {
            IntPoint int_vertex;
         ...
         * @return 返回说明：对应哈希值
         */
        inline uint64 Point2Hash(IntPoint p)
        {
            uint64 hash = 0;
            uint64 tmp;
    ...
    public:
         ...
         * @return 返回说明：无
         */
        inline void SetBoxBoundSet(std ::vector<int32> box_index)
        {
         ...
         * 返回H值查询表
         */
        inline unordered_map<uint32, float64> GetHCostMap()
        {
    ...
    private:
        ...
        /// Hybrid A*
        unordered_map<uint64, Vertex3D> close_map_; // 混合A星算法的close集
        unordered_map<uint64, Vertex3D> open_map_;  // 混合A星算法的open集
        multimap<float64, Vertex3D> open_map_f_;    // 基于f值升序排列的open集优先队列
        unordered_map<uint32, float64> h_cost_map_; // 地图网格的位置和h值的映射关系

        /// A*
        unordered_map<uint64, Node2D> nodes2D_map_; // 存儲所有搜索得到的节点
        set<uint64> nodes2D_set_;                   // open集

        unordered_set<uint32> bound_set_; // 边界的整数坐标形式
        vector<int32> box_index_;
        int RS_num;
        int All;
