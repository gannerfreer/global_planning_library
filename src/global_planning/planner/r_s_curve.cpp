...
namespace HybridAStar
{
   ...
   * @brief RS曲线规划类接口函数
   */
  bool RSCurve::PlanRSPath(const Point start, const Point end,
                           Path &rs_path, PlanRule plan_rule, vehicle_param m_vehicle_prarm)
  {
    m_vehicle_prarm_ = m_vehicle_prarm;
    //    std::cout << "6666m_vehicle_prarm_.radious = " << m_vehicle_prarm_.radious << "\n";
    ...
    if (ReedsSheppGeneration())
    {
      ...
      if (a_star_direction != rs_path.at(0).direction)
        return false;
      return true;
    }
    else
  ...
  {
    if (fabs(opti_rs_path.t) > 0 &&
        fabs(opti_rs_path.u) > 0 &&
        fabs(opti_rs_path.v) > 0)
    {
      std::vector<float64> s;
      s.push_back(opti_rs_path.t);
      s.push_back(opti_rs_path.u);
      s.push_back(opti_rs_path.v);
      if (fabs(opti_rs_path.w) > 1e-6)
        s.push_back(opti_rs_path.w);
      if (fabs(opti_rs_path.x) > 1e-6)
        s.push_back(opti_rs_path.x);
      ...
      for (uint32 i = 0; i < s.size() - 1; i++)
      {
        ...
        else
        {
          if (i == 0)
          {
            int32 temp_direction_rs = (s.at(0) < 0) ? -1 : 1;
            int32 temp_direction_astar = (a_star_direction != Forward) ? -1 : 1;
  ...
  bool RSCurve::ReedsSheppGeneration()
  {
    ...
    if (fabs(opti_rs_path.t) > 1e-6 &&
        fabs(opti_rs_path.u) > 1e-6 &&
        fabs(opti_rs_path.v) > 1e-6)
   ...
   */
  void RSCurve::Interpolate(Point start,
                            Path &rs_path)
  {
    //      std::cout<< "77777m_vehicle_prarm_.step_length =" << m_vehicle_prarm_.step_length << "\n";
    Point temp_point;
    temp_point.x = 0;
    temp_point.y = 0;
    temp_point.z = 0;
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      else
      {
        if (i == 0)
        {
          CalNextPoint(opti_rs_path.t,
                       temp_point.x,
                       temp_point.y,
                       temp_point.angle,
                       opti_rs_path.type.at(i),
                       temp_point,
                       directions);
        ...
        {
          CalNextPoint(opti_rs_path.u,
                       rspoint.at(rspoint.size() - 1).x,
                       rspoint.at(rspoint.size() - 1).y,
                       rspoint.at(rspoint.size() - 1).theta,
                       opti_rs_path.type.at(i),
                       temp_point,
                       directions);
    ...
    CalNextPoint(rspoint.at(rspoint.size() - 1).length, rspoint.at(rspoint.size() - 1).x, rspoint.at(rspoint.size() - 1).y,
                 rspoint.at(rspoint.size() - 1).theta, rspoint.at(rspoint.size() - 1).types, np, directions);
    np.x = np.x * m_vehicle_prarm_.radious + start.x;
    np.y = np.y * m_vehicle_prarm_.radious + start.y;
    np.angle = Mod2pi(np.angle);
    np.direction = directions;
   ...
   * @brief 根据当前点计算下一点
   */
  void RSCurve::CalNextPoint(double s,
                             double x,
                             double y,
                             double th,
                             ReedsSheppPathSegmentType type_,
                             Point &point,
                             MotionDirection &directions)
  {
    if (s > 0)
  ...
  void RSCurve::CCC()
  {
    ...
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRnLn(x, y, phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v)))
    {
      opti_rs_path.set(reeds_shepp_path_type_v.at(0), t, -u, -v, 0, 0, L);
      Lmin = L;
    }
    ...
    if (PlanRule::Normal_Planning == rs_plan_rule && LpRnLn(-x, y, -phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v))) // timeflip
    {
      ...
      ;
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRnLn(x, -y, -phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v))) // reflect
    {
      opti_rs_path.set(reeds_shepp_path_type_v.at(1), t, -u, -v, 0, 0, L);
      Lmin = L;
    }
    ...
    // backwards
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRpLn(x, y, phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v)))
    {
      opti_rs_path.set(reeds_shepp_path_type_v.at(0), t, u, -v, 0, 0, L);
      Lmin = L;
    }
    ...
    if (PlanRule::Normal_Planning == rs_plan_rule && LpRpLn(-x, y, -phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v))) // timeflip
    {
      ...
      ;
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRpLn(x, -y, -phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v))) // reflect
    {
      opti_rs_path.set(reeds_shepp_path_type_v.at(1), t, u, -v, 0, 0, L);
      Lmin = L;
    }
  ...
  void RSCurve::CCSC()
  {
    ...
    double x = new_end.x, y = new_end.y, phi = new_end.angle;
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRmSmLm(x, y, phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v) + 0.5 * M_PI))
    {
    ...
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRmSmRm(x, y, phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v) + 0.5 * M_PI))
    {
    ...
    if (PlanRule::Normal_Planning == rs_plan_rule && LpRmSmRm(-x, y, -phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v) + 0.5 * M_PI)) // timeflip
    {
      ...
      ;
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRmSmRm(x, -y, -phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v) + 0.5 * M_PI)) // reflect
    {
    ...
    // backwards
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRmSmLmBack(x, y, phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v) + 0.5 * M_PI))
    {
    ...
    if (PlanRule::Normal_Planning == rs_plan_rule && LpRmSmLmBack(-x, y, -phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v) + 0.5 * M_PI)) // timeflip
    {
      ...
      ;
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRmSmLmBack(x, -y, -phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v) + 0.5 * M_PI)) // reflect
    {
    ...
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRmSmRmBack(x, y, phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v) + 0.5 * M_PI))
    {
    ...
    if (PlanRule::Normal_Planning == rs_plan_rule && LpRmSmRmBack(-x, y, -phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v) + 0.5 * M_PI)) // timeflip
    {
      ...
      ;
    if (PlanRule::Forward_To_End != rs_plan_rule &&
        PlanRule::Backward_All_Time != rs_plan_rule && LpRmSmRmBack(x, -y, -phi, t, u, v) && Lmin > (L = fabs(t) + fabs(u) + fabs(v) + 0.5 * M_PI)) // reflect
    {
