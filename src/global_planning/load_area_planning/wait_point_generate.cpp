    ...
    for (double i = 0.0; i * delta_steering_angle_ + min_steering <= max_steering; i += 1.0) {
        ...
        if (isinf(R)) {
            ...
            for (double i = min_curve_length_; i <= max_curve_length_; i += delta_curve_length_) {
                ...
                temp_point.path_point.angle = start_point.angle * M_PI / 180.0;
            ...
            for (double i = min_curve_length_; i <= max_curve_length_; i += delta_curve_length_) {
                ...
                temp_end_point.x     = temp_point.path_point.x + 2.0 * cos(temp_point.path_point.angle * M_PI / 180.0);
                temp_end_point.y     = temp_point.path_point.y + 2.0 * sin(temp_point.path_point.angle * M_PI / 180.0);
                temp_end_point.angle = temp_point.path_point.angle;

                temp_point.path_point.angle = temp_point.path_point.angle * M_PI / 180.0;
                if (collision_checker.IsVehicleCollisionWithAll(temp_point.path_point)) {
                    break;
                }
                temp_point.path_point.angle = temp_point.path_point.angle * 180.0 / M_PI;

                temp_end_point.angle = temp_end_point.angle * M_PI / 180.0;
                if (collision_checker.IsVehicleCollisionWithAll(temp_end_point)) {
                    continue;
                }
                temp_end_point.angle = temp_end_point.angle * 180.0 / M_PI;
                ...
                for (const auto& path_point : depart_path) {
                    ...
                    GlobalPlanning::Box2d temp_end_point_box(temp_end_point.x, temp_end_point.y, temp_end_point.angle * M_PI / 180.0, center2front_ + safe_margin_front_, center2rear_ + safe_margin_rear_, center2side_ + safe_margin_2side_);
                    if (temp_end_point_box.IsOverlap(path_point_box)) {
                        collision_with_depart_path = true;
                        break;
                    }
                }
...
double WaitPointGenerator::CalculateGradeAboutLoadPoint(const arc_sample_point& sample_point, const GlobalPlanning::Point& load_point, const GlobalPlanning::Point& closest_point) {
    ...
    cout << "sample_point_info:" << sample_point.path_point.x << "," << sample_point.path_point.y << "," << sample_point.path_point.angle << "  grade_info:" << dist_score << "," << angle_score << "," << (weight_length_ * dist_score + weight_curve_ * angle_score) / (weight_length_ + weight_curve_) << endl;
...
GlobalPlanning::Path WaitPointGenerator::GenerateWaitPointInterface(const GlobalPlanning::Point& load_point, const GlobalPlanning::Path& depart_path, GlobalPlanning::CollisonCheck& collision_checker, FittingPathGenerate::FittingPathGenerator& fitting_path_generator, const GlobalPlanning::Path& in_path, const GlobalPlanning::_VehicleParam& veh_param) {
    ...
    for (auto& point : wait_point_sample_) {
        cout << "计算碰撞前采样点信息：" << point.path_point.x << "," << point.path_point.y << "," << point.path_point.angle << "," << point.grade << endl;
    }
    ...
    for (auto& point : wait_point_sample_) {
        ...
        auto temp_wait_path  = fitting_path_generator.WaitPathGenerateInterface(depart_path, in_path, temp_end_point, collision_checker, veh_param, false);
    ...
    for (auto& point : wait_point_sample_) {
        ...
        temp_end_point.y     = point.path_point.y + 2.0 * sin(point.path_point.angle * M_PI / 180.0);
        temp_end_point.angle = point.path_point.angle;
        GlobalPlanning::Path temp_wait_point;
        temp_end_point.
        if (IsPathCollision(temp_wait_path.first, depart_path, center2front_, center2rear_, center2side_, 0.0, 0.0, 0.0)) {
            cout << "与装载路径碰撞了:" << point.path_point.x << "," << point.path_point.y << "," << point.path_point.angle << endl;
            // if (fabs(point.path_point.x - 159.371 < 0.1) and fabs(point.path_point.y - -820.232 < 0.1)) {
    ...
    // for (auto& point : wait_point_sample_) {
    //     file << point.path_point.x << " " << point.path_point.y << " " << point.path_point.angle << " " << point.grade << endl;
    // }
    // file.close();
    cout << "排序完毕" << endl;
    cout << "wait_point_sample_.size() = " << wait_point_sample_.size() << endl;
    cout << "代价计算完毕" << endl;
    for (auto& point : wait_point_sample_) {
        cout << "计算碰撞后采样点信息：" << point.path_point.x << "," << point.path_point.y << "," << point.path_point.angle << "," << point.grade << endl;
    }
    int i = 0;
    for (const auto& point : wait_point_sample_) {
        ...
        temp_end_point.y     = point.path_point.y + 2.0 * sin(point.path_point.angle * M_PI / 180.0);
        temp_end_point.angle = point.path_point.angle;
        auto wait_path       = fitting_path_generator.WaitPathGenerateInterface(depart_path, in_path, temp_end_point, collision_checker, veh_param, true);
        // ofstream file;
        ...
        if (!wait_path.first.empty()) {
            ...
            temp_end_point.y         = point.path_point.y + 2.0 * sin(point.path_point.angle * M_PI / 180.0);
            temp_end_point.angle     = point.path_point.angle;
            auto start_straight_path = GenerateStraitLine(point.path_point, temp_end_point);
            reverse(start_straight_path.begin(), start_straight_path.end());
            start_straight_path.erase(start_straight_path.end() - 1);
            start_straight_path.insert(start_straight_path.begin(), temp_end_point);
